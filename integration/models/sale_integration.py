...
INVENTORY_DEPENDENT_FIELDS = {
    ...
    'synchronize_all_inventory_periodically'
}
...
class SaleIntegration(models.Model):
    ...
    integration_lang_id = fields.Many2one(
        ...
        string='Default E-Commerce Store Language',
        help=(
    ...
    def is_order_import_enabled(self):
        ...
        Returns True only when the integration is active AND import_order_enabled is True.
        """
        return self.is_active and self.import_order_enabled

    @property
    def is_periodic_inventory_sync_enabled(self):
        """
        Check if periodic inventory synchronization is enabled for this integration.
        Returns True only when the integration is active AND synchronize_all_inventory_periodically is True.
        Note: This is for scheduled/periodic sync, not real-time inventory updates.
        """
        return self.is_active and self.synchronize_all_inventory_periodically
    ...
    def _update_cron_names(self):
        ...
        for integration in self:
            ...
            if integration.receive_orders_cron_id:
                integration.receive_orders_cron_id.name = integration._get_cron_name('Import Orders')

            # Update inventory sync cron name
            if integration.inventory_synchronization_cron_id:
                integration.inventory_synchronization_cron_id.name = integration._get_cron_name('Periodic Inventory Sync')  # NOQA
    ...
    def write(self, vals):
        ...
        if 'skip_inventory_field_values_update' in ctx:
            return res
        ...
        # 3. Update crons
        if any(field in vals for field in ['synchronize_all_inventory_periodically', 'import_order_enabled']):
            self._update_crons_activity()

        # 4. Update cron names if integration name or type_api changed
        if 'name' in vals or 'type_api' in vals:
            self._update_cron_names()

        # 5. Normalize dependency fields
    ...
    def _update_inventory_field_values(self):
        ...
        - If both 'export_inventory_job_enabled' and 'synchronize_all_inventory_periodically'
        ...
        for rec in self:
            ...
            if not rec.export_inventory_job_enabled and not rec.synchronize_all_inventory_periodically:
                if rec.update_stock_for_manufacture_boms:
                    updates['update_stock_for_manufacture_boms'] = False

            if updates:
                rec.with_context(skip_inventory_field_values_update=True).write(updates)
    ...
    def write_settings_fields(self, vals):
        ...
        if fields_list_to_add:
            res = self.with_context(
                skip_write_settings_fields=True,
    ...
    def action_import_product_from_external(self):
        ...
        This method will import products from the e-commerce system and automatically
        ...
        """
        action, tmpl_hub = self._validate_product_templates_with_action(False)
        if action:
            return action
    ...
    def action_create_products_in_odoo(self):
        action, __ = self._validate_product_templates_with_action(False)  # TODO, maybe it's not necessary
        if action:
            return action
    ...
    def _prepare_calculation_qty_with_bom(self, product_id, locations):
        ...
        if skipped_locations:
            names = ', '.join(f'{l.display_name} (ID: {l.id})' for l in skipped_locations)
            report.append(_(
    ...
    def integrationApiProductsValidationTest(self):
        ...
        if validation_results:
            message_wizard = self.env['message.wizard'].create({
                ...
                'message_html': validation_results,
            })
    ...
    def import_single_customer(self, external_customer_id):
        ...
        billing_addresses = (
            [a for a in addresses if a.get('type') == 'invoice'] +
    ...
    def filter_received_orders(self, external_orders_data_list: list):
        ...
        if hasattr(self, filter_method_name):
            ...
            return filter_method(external_orders_data_list)
        else:
    ...
    def is_importable_order_status(self, statuses: list[str]) -> None:
        ...
        """
        raise IntegrationNotImplementedError(_('Each connector has its own status filtering mechanism and format.'))
    ...
    def integrationApiExportInventory(self):
        ...
        if not self.is_periodic_inventory_sync_enabled:
            _logger.info(
    ...
    def get_order_url(self, external_order_id):
        ...
        return adapter.get_order_url(external_order_id)
    ...
    def _save_log(self, vals):
        try:
            ...
            _logger.error(f'Failed to save log: {ex}')
