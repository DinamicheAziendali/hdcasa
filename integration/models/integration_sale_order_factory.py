    ...
    def _try_get_odoo_product(self, integration, line, force_create=False):
        ...
        if not external_template or complex_variant_code not in external_variants.mapped('code'):
            ...
            raise ValidationError(
                _(
                    f'The order contains a line item with missing product '
                    'details (either product ID or SKU is empty).\n\n'
                    f'Missing Product Information:\n'
                    f'- Product Template ID: {template_code}\n'
                    f'- Product Variant ID: {variant_code}\n\n'
                    ...
                    '3. Check if the product with Template ID "{template_code}" and Variant ID "{variant_code}" '
