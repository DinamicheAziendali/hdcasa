...
class StockQuant(models.Model):
    ...
    def trigger_export(self):
        ...
        product_integrations = self.env['sale.integration'].get_integrations('export_inventory')
        if not product_integrations:
            return
        ...
        for template in templates:
            if template.company_id:
                integrations = product_integrations.filtered(lambda x: x.company_id == template.company_id)
            else:
