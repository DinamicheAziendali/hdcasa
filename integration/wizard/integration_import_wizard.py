...
class IntegrationImportWizard(models.TransientModel):
    ...
    def action_continue_to_product_import(self):
        if self.import_in_background and not self.jobs_done:
            raise UserError('Background jobs are still running. Please wait for them to finish.')
    ...
    def action_run_product_import(self):
        ...
        if not self.integration_id:
            raise UserError('No integration selected.')
    ...
    def action_run_update_import(self):
        ...
        if not entity:
            raise UserError('No entity selected. Please select an entity to import.')
        ...
        if not entity.method_name:
            raise UserError(
                f'Import for entity "{entity.name}" can\'t be run because it is not implemented yet. '
