...
class DaXlsxPinto(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, picking_ids):
        ...
        text_style = workbook.add_format({"bold": False, "font_size": 9, "font_color": "#000000", "align": "left"})
        title_style = workbook.add_format({"bold": True, "bottom": 1})
        number_style = workbook.add_format({"bold": False, "font_size": 9, "font_color": "#000000", "align": "right"})
        monetary_style = workbook.add_format({"bold": False, "font_size": 9, "font_color": "#000000", "align": "right", "num_format": "€ #,##0.00"})
        sheet_title = [
            _("Riferimento"),
        ...
        for picking in picking_ids:
            ...
            for move in picking.move_ids:
                for move_line in move.move_line_ids:
                    ...
                    product_supplier_code = self.get_supplier_code_product(move_line.product_id, picking.partner_id)
                    product_supplier_price = self.get_supplier_cost_product(move_line.product_id, picking.partner_id)
                    ...
                    sheet.write(i, 10, datetime.now().date().strftime('%d/%m/%Y'), text_style)
                    i += 1
            if writed:
    ...
    def get_supplier_code_product(self, product, supplier):
        if product and supplier:
            ...
            if supplier in product_supplier_ids:
                product_supplier = product.seller_ids.filtered(lambda p: p.partner_id == supplier)
