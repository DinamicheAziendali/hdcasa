...
class TestIsImportableOrderStatusShopify(OdooIntegrationInit):
    ...
    def test_shopify_special_status_any(self):
        """Test Shopify integration with 'any' special status."""
        self.shopify_integration.set_settings_value('receive_order_financial_statuses', 'any')
        self.shopify_integration.set_settings_value('receive_order_fulfillment_statuses', 'any')

        # Any status combination should be allowed
        for financial_status in ['paid', 'pending', 'authorized']:
            for fulfillment_status in ['fulfilled', 'unfulfilled', 'partial']:
                result = self.shopify_integration.is_importable_order_status([financial_status, fulfillment_status])
                self.assertTrue(result, f"Status combination {financial_status}/{fulfillment_status} should be allowed")

    def test_shopify_special_status_shipped_mapping(self):
        """Test Shopify integration with 'shipped' special status mapping to 'fulfilled'."""
        self.shopify_integration.set_settings_value('receive_order_financial_statuses', 'paid')
        self.shopify_integration.set_settings_value('receive_order_fulfillment_statuses', 'shipped')

        # 'fulfilled' status should be allowed when 'shipped' is configured
        result = self.shopify_integration.is_importable_order_status(['paid', 'fulfilled'])
        self.assertTrue(result)
