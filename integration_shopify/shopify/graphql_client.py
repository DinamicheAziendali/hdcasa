...
class ShopifyGraphQL:
    ...
    def is_media_file_ready(self, image_gid: str):
        image = self._is_media_file_ready(image_gid)
        return image.get('status') == 'READY'

    @extract_node('data.node', dict)
    def _is_media_file_ready(self, image_gid: str):
        ...
        return self.execute(schema, variables={'id': image_gid})
