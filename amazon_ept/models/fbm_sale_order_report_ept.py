...
class FbmSaleOrderReportEpt(models.Model):
    ...
    def auto_import_unshipped_order_report(self, seller):
        ...
        if seller.id:
            sale_order_report = self.create(
                {'report_type': ReportType.GET_FLAT_FILE_ORDER_REPORT_DATA.value,
                 'seller_id': seller.id,
                 'state': 'draft',
    ...
    def default_get(self, field):
        ...
        if not field:
            return res
        res.update({'report_type': ReportType.GET_FLAT_FILE_ORDER_REPORT_DATA.value})
        return res
    ...
    def prepare_amazon_request_report_kwargs(self, seller):
        ...
        if kwargs:
            ...
            kwargs.update({'report_type': ReportType.GET_FLAT_FILE_ORDER_REPORT_DATA.value,
        ...
        for row in reader:
            ...
            if order:
                ...
                if not order.buyer_requested_cancellation:
                    order.buyer_requested_cancellation = ast.literal_eval(row.get('is-buyer-requested-cancellation',
                                                                      'false').capitalize())
                continue
    ...
    def prepare_order_dict(order_dict, amz_ref, instance_id, row, business_prime_dict):
        ...
        fbm_order_dict = {
            ...
            'buyer_requested_cancellation': ast.literal_eval(row.get('is-buyer-requested-cancellation', 'false').capitalize())
            }
