...
class VcsTaxReport(models.Model):
    ...
    def process_vcs_tax_report_file(self):
        ...
        for row in reader:
            ...
            if not amz_prod:
                message = ('While processing the VCS report file, the system tried to search the Amazon product with '
                           'seller ref: %s but did not find it.\n'
                           'Action Items:\n'
                           '-Create the product in Amazon Products. To create the product, you can use the "Map Products"'
                           ' functionality from the Operation Wizard.\n'
                           '-Reprocess the report from: Amazon → VAT Calculation Service Reports → Re-Process File') % sku
                common_log_line_obj.create_common_log_line_ept(
                    message=message, model_name='amazon.vcs.tax.report.ept', default_code=sku, module='amazon_ept',
                    operation_type='import', res_id=self.id, mismatch_details=True,
                    amz_seller_ept=self.seller_id and self.seller_id.id or False)
    ...
    def check_vcs_report_file_data_ept(row, line_no):
        ...
        message = ''
        if not order_id:
            message = ('While processing the VCS report file, Order ID is not set at file index line number %d.\n'
                       'Action Items:\n'
                       '-Check and update the Order ID in the file.') % line_no
            return message
        if not marketplace_id:
            message = ('While processing the VCS report file, Marketplace ID for Amazon order ref: %s is not set at file index line number %d.\n'
                       'Action Items:\n'
                       '-Update the Marketplace ID in the file.') % (order_id, line_no)
            return message
        if not invoice_type:
            message = ('While processing the VCS report file, Transaction Type for Amazon order ref: %s is not set at file index line number %d.\n'
                       'Action Items:\n'
                       '-Set the appropriate Transaction Type in the file.') % (order_id, line_no)
            return message
        if not sku:
            message = ('While processing the VCS report file, SKU for Amazon order ref: %s is not set at file index line number %d.\n'
                       'Action Items:\n'
                       '-Add the correct SKU to the file.') % (order_id, line_no)
            return message
        if invoice_type != 'SHIPMENT' and not qty:
            message = ('While processing the VCS report file, received Transaction Type is %s but Quantity is not '
                       'set in the file for Amazon order ref: %s at file index line number %d.\n'
                       'Action Items:\n'
                       "-Set the Quantity value for the 'invoice' line.") % (invoice_type, order_id, line_no)
            return message
        if not ship_from_country:
            message = ('While processing the VCS report file, Ship From Country is not set for Amazon order ref: %s at '
                       'file index line number %d.\n'
                       'Action Items:\n'
                       '-Provide the Ship From Country in the file.') % (order_id, line_no)
            return message
        return message
    ...
    def get_amz_vcs_invoices(self):
        """
        Opens the list view of Invoices.
        :return: ir.actions.act_window()
        """
        return {
