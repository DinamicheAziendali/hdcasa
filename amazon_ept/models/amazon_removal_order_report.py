...
class AmazonRemovalOrderReportHistory(models.Model):
    ...
    def default_get(self, field):
        ...
        if not field:
            return res
        res.update({'report_type': ReportType.GET_FBA_FULFILLMENT_REMOVAL_ORDER_DETAIL_DATA.value})
        return res
    ...
    def process_removal_order_report(self):
        ...
        if not self.seller_id.instance_ids.filtered(lambda l: l.is_allow_to_create_removal_order):
            message = ('Configuration is not enabled to create or import removal orders for the specified seller.\n'
            'Action items:\n'
            '-Go to: Amazon → Configuration → Settings → Select Seller → Enable "Allow Create Removal Order"'
            ' in FBA.\n'
            '-Re-import the report using the operation wizard.')
            if not self._context.get('is_auto_process', False):
                raise UserError(_(message))
            log_line_obj.create_common_log_line_ept(
                message=message, model_name=AMZ_REMOVAL_ORDER_REPORT_HISTORY,
    ...
    def amz_prepare_disposal_and_removal_line_dict(self, existing_order, rows, disposal_line_dict,
        ...
        for row in rows:
            ...
            if not amazon_removal_order_config:
                message = ("Removal order configuration with Disposition Type '{}' not found for Amazon order reference: {}.\n"
                           "Action items:\n"
                           "-Go to: Amazon → Configuration → Removal Orders.\n"
                           "-Set up the {} Disposition Type.\n"
                           "-Re-process the file from: Amazon → FBA → Removal Order Reports.").format(
                    row.get('order-type', ''), row.get('order-id', ''), row.get('order-type', ''))
                log_line_obj.create_common_log_line_ept(
        ...
        for order_id, rows in list(order_dict.items()):
            ...
            for row in rows:
                ...
                if not amazon_product:
                    message = ("Product not found for Seller SKU: {} (Order ref: {}).\n"
                               "Action items:\n"
                               "-Create a new product in Odoo with internal reference: {}, OR\n"
                               "-Assign the SKU to an existing product.\n"
                               "-Re-process the file from: Amazon → FBA → Removal Order Reports.").format(row.get('sku', ''), order_id, row.get('sku', ''))
                    log_line_obj.create_common_log_line_ept(
                        message=message, model_name=AMZ_REMOVAL_ORDER_REPORT_HISTORY, module='amazon_ept',
                        operation_type='import', res_id=self.id, mismatch_details=True,
                        amz_seller_ept=self.seller_id and self.seller_id.id or False,
                ...
                if float(row.get('requested-quantity', 0.0)) <= 0.0:
                    message = ("Requested quantity missing for order ref: {} with Seller SKU: {}.\n"
                               "Action items:\n"
                               "-Download the file and check the quantity for the given order and SKU.").format(order_id, row.get('sku', ''))
                    log_line_obj.create_common_log_line_ept(
                        message=message, model_name=AMZ_REMOVAL_ORDER_REPORT_HISTORY, module='amazon_ept',
                        operation_type='import', res_id=self.id, mismatch_details=True,
                        amz_seller_ept=self.seller_id and self.seller_id.id or False,
    ...
    def update_return_or_removal_line_dict_ept(self, key, row, disposal_line_dict, return_line_dict,
        ...
        if row.get('order-type', '') == 'Disposal':
            ...
            message = 'Skipped order reference: {} as the received order type is "{}".'.format(row.get('order-id', ''), row.get('order-type', ''))
            log_line_obj.create_common_log_line_ept(
                message=message, model_name=AMZ_REMOVAL_ORDER_REPORT_HISTORY, module='amazon_ept',
    ...
    def check_amazon_order_exist_order_not(self, row):
        ...
        if not existing_order and order_status == 'Cancelled':
            message = ("Amazon order reference: {} not found in Odoo.\n"
                       "Action items:\n"
                       "-The order should exist in Odoo.\n"
                       "-You can import it from the operation wizard or skip it if not required.").format(order_id)
            log_line_obj.create_common_log_line_ept(
                message=message, model_name=AMZ_REMOVAL_ORDER_REPORT_HISTORY, module='amazon_ept',
                operation_type='import', res_id=self.id, mismatch_details=True,
                amz_seller_ept=self.seller_id and self.seller_id.id or False,
            ...
            message = ("Multiple orders found in Odoo with Amazon order reference: {}.\n"
                       "Action items:\n"
                       "-The order should be unique in Odoo.\n"
                       "-Take manual action to cancel or clean up duplicate entries.").format(order_id)
            log_line_obj.create_common_log_line_ept(
                message=message, model_name=AMZ_REMOVAL_ORDER_REPORT_HISTORY, module='amazon_ept',
                operation_type='import', res_id=self.id, mismatch_details=True,
                amz_seller_ept=self.seller_id and self.seller_id.id or False,
    ...
    def amz_removal_canceled_qty_ept(self, row, picking_vals, quantity):
        ...
        if qty > 0.0:
            moves = self.amz_get_stock_move_from_picking_ept(picking_vals.get('product_id', False),
                                                             ...
                                                             ['done', 'cancel'])
            if not moves:
                message = ('Stock move not found for Amazon order reference: {} with SKU: {}.\n'
                           'Action items:\n'
                           '-Stock move should exist for this order in Odoo.\n'
                           ).format(picking_vals.get('order', '').name, sku)
                log_line_obj.create_common_log_line_ept(
                    message=message, model_name=AMZ_REMOVAL_ORDER_REPORT_HISTORY, module='amazon_ept',
                    operation_type='import', res_id=self.id, mismatch_details=True,
                    amz_seller_ept=self.seller_id and self.seller_id.id or False,
    ...
    def amz_prepare_removal_lines_prd_wise_qty_ept(removal_lines):
        ...
        for line in removal_lines:
            ...
            canceled_qty = float(line.get('cancelled-quantity', 0.0) or 0.0)
            disposed_qty = float(line.get('disposed-quantity', 0.0) or 0.0)
            order_id = line.get('order-id', '')
            if key in prd_wise_qty:
                shipped_qty += prd_wise_qty.get(key, {}).get('shipped-quantity', 0.0)
                canceled_qty += prd_wise_qty.get(key, {}).get('cancelled-quantity', 0.0)
                disposed_qty += prd_wise_qty.get(key, {}).get('disposed-quantity', 0.0)
                prd_wise_qty.get(key, {}).update({'shipped-quantity': shipped_qty,
                                                  'cancelled-quantity': canceled_qty,
                                                  'disposed-quantity': disposed_qty})
            else:
                prd_wise_qty.update({key: {'shipped-quantity': shipped_qty, 'cancelled-quantity': canceled_qty,
                                           'disposed-quantity': disposed_qty, 'fnsku': fn_sku,
    ...
    def find_amazon_product_for_process_removal_line(self, line, instance):
        ...
        if not amazon_product:
            log_line_obj.create_common_log_line_ept(
                message='Product not found for SKU: {} & ASIN: {}.\n'
                        'Action items:\n'
                        '-Create a new product with internal reference: {}, OR\n'
                        '-Map it to an existing product in Odoo using the “Map Product” functionality.\n'
                        '-Re-process the file from: Amazon → FBA → Removal Order Reports.'.format(sku, asin, sku),
    ...
    def auto_import_removal_order_report(self, args={}):
        ...
        if seller_id:
            ...
            rem_report = self.create({'report_type': ReportType.GET_FBA_FULFILLMENT_REMOVAL_ORDER_DETAIL_DATA.value,
