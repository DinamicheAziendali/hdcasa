...
class ActiveProductListingReportEpt(models.Model):
    ...
    def request_report(self):
        ...
        kwargs.update({'emipro_api': 'create_marketplace_report_sp_api',
                       'report_type': self.report_type or ReportType.GET_MERCHANT_LISTINGS_DATA.value,
    ...
    def sync_products(self):
        ...
        reader, headers = self.amz_read_active_listing_report()
        process_count = 0
            ...
            if amazon_product_id:
                ...
                if len(odoo_product) > 1:
                    ...
                    message = ("Multiple Odoo products found with the same Internal Reference: %s.\n"
                               "Action Items:\n"
                               "-Search for products in Odoo with Internal Reference %s.\n"
                               "-Archive unnecessary or duplicate products.\n"
                               "-Ensure that only one unique product exists with Internal Reference %s.\n"
                               "-Reprocess the file from: Amazon → Products → Active Product List Reports.") % (
                                  seller_sku, seller_sku, seller_sku)
                    common_log_line_obj.create_common_log_line_ept(
    ...
    def amz_find_odoo_product_ept(self, seller_sku):
        """
        Define this method for find odoo product.
    ...
    def amz_read_active_listing_report(self, is_utf_code=False):
        """
        Define this method for read active listing report file as per given utf-code.
        :param: is_utf_code: True/False
        :return: csv dict reader, list of headers []
        """
        if is_utf_code:
            imp_file = StringIO(base64.b64decode(self.attachment_id.datas).decode('utf-8-sig'))
        else:
        ...
        if odoo_product:
            ...
            if self.auto_create_product:
                if not row.get('item-name', ''):
                    message = ("System attempted to search for an Odoo product using Seller SKU %s, but no product"
                               " was found. Based on the configuration, it attempted to create a new product, but the"
                               " item name was missing in the file.\n"
                               "Action items:\n"
                               "-Open the Active Product List Report and locate the entry for Seller SKU %s.\n"
                               "-Ensure the item name is correctly set in the file.\n"
                               "-Reprocess the file from:Amazon → Products → Active Product List Reports.") % (
                                  seller_sku, seller_sku)
                    is_mismatch = True
                else:
