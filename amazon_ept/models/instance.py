...
class AmazonInstanceEpt(models.Model):
    ...
    is_expiration = fields.Boolean(default=False)
    is_manufacturing_lot_code = fields.Boolean(default=False)
    is_inbound_box_information_known = fields.Boolean(string="Is Box Information Known?", default=False)
    is_marketplace_workflow = fields.Boolean("Marketplace Workflow", related="seller_id.is_marketplace_auto_workflow")
    fbm_marketplace_workflow_id = fields.Many2one('sale.workflow.process.ept', string='AutoWorkflow(FBM)',
    ...
    def get_fba_total_orders(record, fulfillment_by_where_clause):
        ...
        def orders_of_current_week(record, fulfillment_by_where_clause):
            query = sql.SQL("""select id from sale_order where date(date_order) >= 
            (select date_trunc('week', date(current_date))) and amz_instance_id= %s and state in ('sale','done') 
            and amz_fulfillment_by = 'FBA' {} order by date(date_order)""").format(sql.SQL(fulfillment_by_where_clause))
        ...
        def orders_of_current_month(record, fulfillment_by_where_clause):
            query = sql.SQL("""select id from sale_order where date(date_order) >= 
            (select date_trunc('month', date(current_date))) and amz_instance_id= %s and state in ('sale','done') 
            and amz_fulfillment_by = 'FBA' {} order by date(date_order)""").format(sql.SQL(fulfillment_by_where_clause))
            record._cr.execute(query, (record.id,))
            return record._cr.dictfetchall()
        ...
        def orders_of_current_year(record, fulfillment_by_where_clause):
            query =sql.SQL("""select id from sale_order where date(date_order) >= 
            (select date_trunc('year', date(current_date))) and amz_instance_id= %s and state in ('sale','done') and 
            amz_fulfillment_by = 'FBA' {} order by date(date_order)""").format(
                sql.SQL(fulfillment_by_where_clause))
            record._cr.execute(query, (record.id,))
            return record._cr.dictfetchall()

        def orders_of_all_time(record, fulfillment_by_where_clause):
            query = sql.SQL("""select id from sale_order where amz_instance_id = %s and state in ('sale','done') and 
            amz_fulfillment_by = 'FBA' {}""").format(sql.SQL(fulfillment_by_where_clause))
            record._cr.execute(query, (record.id,))
            return record._cr.dictfetchall()
    ...
    def get_compare_data(record, fulfillment_by_where_clause):
        ...
        def get_compared_week_data(record, fulfillment_by_where_clause):
            ...
            current_week_query = sql.SQL("""select sum(amount_untaxed) as current_week from sale_order 
            where date(date_order) >= (select date_trunc('week', date(current_date))) and 
            amz_instance_id=%s and state in ('sale','done') {}""").format(sql.SQL(fulfillment_by_where_clause))
            ...
            # Previous week data
            previous_week_query =sql.SQL("""select sum(amount_untaxed) as previous_week from sale_order
            where date(date_order) between (select date_trunc('week', current_date) - interval '7 day')
            and (select date_trunc('week', (select date_trunc('week', current_date) - interval '7 day')) + interval %s)
            and amz_instance_id=%s and state in ('sale','done') {}""").format(sql.SQL(fulfillment_by_where_clause))
            record._cr.execute(previous_week_query, (f'{day_of_week} day', record.id,))
        ...
        def get_compared_month_data(record, fulfillment_by_where_clause):
            ...
            current_month_query = sql.SQL("""select sum(amount_untaxed) as current_month from sale_order 
            where date(date_order) >= (select date_trunc('month', date(current_date))) and 
            amz_instance_id=%s and state in ('sale','done') {}""").format(sql.SQL(fulfillment_by_where_clause))
            ...
            # Previous week data
            previous_month_query = sql.SQL("""select sum(amount_untaxed) as previous_month from sale_order where date(date_order) 
            between (select date_trunc('month', current_date) - interval '1 month') and (select date_trunc('month', (
            select date_trunc('month', current_date) - interval '1 month')) + interval %s) and 
            amz_instance_id=%s and state in ('sale','done') {}""").format(sql.SQL(fulfillment_by_where_clause))
            record._cr.execute(previous_month_query, (f'{day_of_month} days', record.id,))
        ...
        def get_compared_year_data(record, fulfillment_by_where_clause):
            ...
            current_year_query = sql.SQL("""select sum(amount_untaxed) as current_year from sale_order 
            ...
            # Previous week data
            previous_year_query = sql.SQL("""select sum(amount_untaxed) as previous_year from sale_order where 
            date(date_order) between (select date_trunc('year', date(current_date) - interval '1 year')) and 
            (select date_trunc('year', date(current_date) - interval '1 year') + interval %s) 
            and amz_instance_id=%s and state in ('sale','done') {}""").format(sql.SQL(fulfillment_by_where_clause))
            record._cr.execute(previous_year_query, (f'{delta} days', record.id,))
            previous_data = record._cr.dictfetchone()
    ...
    def get_graph_data(record, fulfillment_by_where_clause):
        ...
        def get_current_week_date(record, fulfillment_by_where_clause):
            query = sql.SQL("""SELECT to_char(date(d.day),'DAY'), t.amount_untaxed as sum
                                FROM  (
                                   SELECT day
                                ...
                                (SELECT date(date_order)::date AS day, sum(amount_untaxed) as amount_untaxed
                                   ...
                                   + interval '6 days')
                                   AND amz_instance_id=%s and state in ('sale','done') {}
                                   GROUP  BY 1
                                   ) t USING (day)
                                ORDER  BY day""").format(sql.SQL(fulfillment_by_where_clause))
            record._cr.execute(query, (record.id,))
            return record._cr.dictfetchall()

        def graph_of_current_month(record, fulfillment_by_where_clause):
            query = sql.SQL("""select EXTRACT(DAY from date(date_day)) :: integer,sum(amount_untaxed) from (
                        SELECT 
                        ...
                        GROUP  BY 1
                        ORDER  BY 1""").format(sql.SQL(fulfillment_by_where_clause))
            record._cr.execute(query, (record.id,))
            return record._cr.dictfetchall()

        def graph_of_current_year(record, fulfillment_by_where_clause):
            query = sql.SQL("""select TRIM(TO_CHAR(DATE_TRUNC('month',month),'MONTH')),sum(amount_untaxed) from
                                ...
                                GROUP  BY foo.month
                                order by foo.month""").format(sql.SQL(fulfillment_by_where_clause))
            record._cr.execute(query, (record.id,))
            return record._cr.dictfetchall()

        def graph_of_all_time(record, fulfillment_by_where_clause):
            query = sql.SQL("""select TRIM(TO_CHAR(DATE_TRUNC('month',date_order),'YYYY-MM')),sum(amount_untaxed)
                                from sale_order where amz_instance_id = %s and state in ('sale','done') {}
                                group by DATE_TRUNC('month',date_order) 
                                order by DATE_TRUNC('month',date_order)""").format(sql.SQL(fulfillment_by_where_clause))
            record._cr.execute(query, (record.id,))
            return record._cr.dictfetchall()
    ...
    def get_fbm_total_orders(record, fulfillment_by_where_clause):
        ...
        def orders_of_current_week(record, fulfillment_by_where_clause):
            query = sql.SQL("""select id from sale_order where date(date_order) >= 
            (select date_trunc('week', date(current_date))) and amz_instance_id= %s and state in ('sale','done') 
            and amz_fulfillment_by = 'FBM' {} order by date(date_order)""").format(sql.SQL(fulfillment_by_where_clause))
            record._cr.execute(query, (record.id,))
            return record._cr.dictfetchall()
        ...
        def orders_of_current_year(record, fulfillment_by_where_clause):
            query = sql.SQL("""select id from sale_order where date(date_order) >= 
            (select date_trunc('year', date(current_date))) and amz_instance_id= %s and state in ('sale','done') 
            and amz_fulfillment_by = 'FBM' {} order by date(date_order)""").format(sql.SQL(fulfillment_by_where_clause))
            record._cr.execute(query, (record.id, ))
            return record._cr.dictfetchall()
        ...
        def orders_of_all_time(record, fulfillment_by_where_clause):
            query = sql.SQL("""select id from sale_order where amz_instance_id = %s and state in ('sale','done') and 
            amz_fulfillment_by = 'FBM' {}""").format(sql.SQL(fulfillment_by_where_clause))
            record._cr.execute(query, (record.id,))
            return record._cr.dictfetchall()
