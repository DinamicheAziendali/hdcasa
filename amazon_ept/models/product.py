...
class AmazonProductEpt(models.Model):
    ...
    def export_stock_levels(self, instance):
        ...
        :param : instance : amazon.instance.ept() arguments relocates instance of amazon
        ...
        export_data= self.process_export_stock_message_info_ept(instance, product_ids.ids, amazon_products, warehouse_ids)
        if export_data:
            self.process_amazon_export_stock_dict_ept(instance, export_data)
        return True
    ...
    def export_amazon_stock_levels_operation(self, instance):
        ...
        export_data = self.process_export_stock_message_info_ept(instance, product_ids.ids, amazon_products.ids,
                                                                 warehouse_ids)
        if export_data:
            self.process_amazon_export_stock_dict_ept(instance, export_data)
        return True
    ...
    def process_export_stock_message_info_ept(self, instance, product_ids, amazon_products_ids, warehouse_ids):
        ...
        :param : instance : amazon.instance.ept() arguments relocates instance of amazon
        :param : product_ids : [] arguments relocates product listing id of odoo
        :param : amazon_products_ids : [] arguments relocates product listing id of amazon
        :param : warehouse_ids : [] arguments relocates warehouses of amazon
        :return : [] prepare message for export stock data
        """
        ...
        if product_listing_stock:
            ...
            for amazon_product_id in amazon_products_ids:
                ...
                product_data = self.prepare_export_stock_level_dict_operation(amazon_product, stock, message_id)
                export_data.append(product_data)
                message_id += 1
        return export_data

    def process_amazon_export_stock_dict_ept(self, instance, export_data):
        """
        This method will prepare the export json data for exporting stock to amazon
        ...
        kwargs = self.get_amazon_product_request_data_ept(instance, data, 'amazon_submit_feeds_sp_api')
        kwargs.update({'feed_type':'JSON_LISTINGS_FEED'})
        response = iap_tools.iap_jsonrpc(DEFAULT_ENDPOINT, params=kwargs, timeout=1000)
    ...
    def process_amazon_export_stock_response_ept(self, instance, data, response):
        ...
        if response.get('error', False):
            common_log_line_obj.create_common_log_line_ept(
                message=response.get('error', {}), model_name=AMAZON_PRODUCT_EPT,
                fulfillment_by='FBM', module='amazon_ept', operation_type='export',
                amz_instance_ept=instance and instance.id or False, mismatch_details=True,
    ...
    def prepare_export_stock_level_dict_operation(self, amazon_product, actual_stock, message_id):

        """
        This method prepared data of export stock value.
        :param amazon_product: amazon.product.ept() arguments relocates product of amazon.
        :param actual_stock : float() stock
        :param message_id: int() arguments relocates message id.
        :return: This method return export data {}
        """
        seller_sku = html.escape(amazon_product['seller_sku'])
        stock = self.stock_ept_calculation(actual_stock, amazon_product['fix_stock_type'],
        ...
        instance = amazon_product.instance_id
        ...
        export_data = {'messageId': message_id, 'sku': seller_sku, 'operationType': 'PATCH',
					   'productType': 'PRODUCT', 'patches': [{'op': 'replace',
															  'path': '/attributes/fulfillment_availability',
															  'value': [{'fulfillment_channel_code': 'DEFAULT',
																		 'quantity': stock,
																		 'lead_time_to_ship_max_days': int(
    ...
    def update_price(self, instance):
        """
        This Method relocates create data for update price in amazon.
        :param instance: amazon.instance.ept() arguments relocates instance of amazon.
        ...
        export_price_data = []
        for amazon_product in self:
            data = self.update_price_dict(instance, amazon_product, message_id)
            message_id += 1
            export_price_data.append(data)
        if export_price_data:
            data = {'header': {'sellerId': instance.merchant_id,
                               'version': '2.0',
                               'issueLocale': 'en_US'
                               },
                    'json_feed':True, 'messages': export_price_data
                    }
            kwargs = self.get_amazon_product_request_data_ept(instance, data, 'amazon_submit_feeds_sp_api')
            kwargs.update({'feed_type': 'JSON_LISTINGS_FEED'})
            response = iap_tools.iap_jsonrpc(DEFAULT_ENDPOINT, params=kwargs, timeout=1000)
    ...
    @staticmethod
    def update_price_dict(instance, amazon_product, message_id):
        """
        This Method relocates Prepare price dictionary for exporting amazon.
        :param instance: amazon.instance.ept() arguments relocates instance of amazon.
        :param amazon_product: amazon.product.ept() arguments relocates product listing of amazon.
        :param message_id: int() arguments relocates message of amazon.
        :return: {}This Method return price data
        """
        product = amazon_product.product_id
        price = instance.pricelist_id._get_product_price(product=product, quantity=1.0, uom=product.uom_id)
        price = price and round(price, 2) or 0.0
        seller_sku = html.escape(amazon_product.seller_sku)
        price_data = {'messageId': message_id,
                      'sku': seller_sku,
                      'operationType': 'PATCH', 'productType': 'PRODUCT',
    ...
    def _name_search(self, name, domain=None, operator='ilike', limit=None, order=None):
        ...
        if name:
            domain.extend(['|', '|', '|', ('seller_sku', operator, name), ('name', operator, name),
...
class ProductProduct(models.Model):
    ...
    is_mapped_with_amz = fields.Boolean(compute="_compute_is_amazon_mapped", search="_search_is_amazon_mapped",
                                        compute_sudo=True)
    ...
    def _search_is_amazon_mapped(self, operator, value):
        amz_product_obj = self.env[AMAZON_PRODUCT_EPT]
        odoo_product = amz_product_obj.search([]).product_id
        product_mapped_ids = [rec.id for rec in odoo_product if rec.is_mapped_with_amz]
        return [('id', 'in', product_mapped_ids)]
