...
class StockAdjustmentReportHistory(models.Model):
    ...
    def default_get(self, field):
        ...
        if field:
            res.update({'report_type': ReportType.GET_LEDGER_DETAIL_VIEW_DATA.value})
        return res
    ...
    def process_stock_adjustment_report(self):
        ...
        if not self.attachment_id:
            message = ("Attachment not found in the Stock Adjustment report.\n"
                       "Action items:\n"
                       "-Check if the attachment exists in the Stock Adjustment chatter.\n"
                       "-You can check the status and download the report from: Amazon → FBA → Stock Adjustment.\n"
                       "-Once the attachment is found in the chatter, you can proceed with processing the file.")
            if not self._context.get('is_auto_process', False):
        ...
        if not self.seller_id:
            message = ("Seller is not defined while processing the Stock Adjustment report.\n"
                       "Action items:\n"
                       "-Seller must be set in the Stock Adjustment report.")
            if not self._context.get('is_auto_process', False):
                raise UserError(_(message))
            common_log_lines_obj.create_common_log_line_ept(
                message=message, model_name=AMAZON_STOCK_ADJUSTMENT_REPORT_HISTORY,
        ...
        if self.amz_check_report_required_headers('Reason', reader.fieldnames,
                                                  self._context.get('is_auto_process', False)):
            return True
        group_wise_lines_list, partially_processed = self._prepare_group_wise_lines_list_ept(reader)
    ...
    def _prepare_group_wise_lines_list_ept(self, reader):
        """
        Prepare stock adjustment list as per it's Groups configurations.
        :param: reader: dict reader
        ...
        for row in reader:
            ...
            if not code:
                ...
                message = ('%s code not found in the Stock Adjustment reasons.\n'
                           'Action items:\n'
                           '-Set the %s code in: Amazon → Configuration → FBA Stock Adjustment Reasons.\n'
                           '-Reprocess the file from: Amazon → FBA → Stock Adjustment.') % (reason, reason)
            ...
            if len(code.ids) > 1:
                partially_processed = True
                message = ('Multiple %s reason codes found while processing the Stock Adjustment report.\n'
                           'Action items:\n'
                           '-Only one unique %s reason should be configured in: Amazon → Configuration → FBA Stock '
                           'Adjustment Reasons.\n'
                           '-Reprocess the file from: Amazon → FBA → Stock Adjustment.') % (reason, reason)
                common_log_line_obj.create_common_log_line_ept(
                    message=message, model_name=AMAZON_STOCK_ADJUSTMENT_REPORT_HISTORY, mismatch_details=True,
            ...
            if not config:
                partially_processed = True
                message = ('%s group not found for the specific seller in FBA Stock Adjustment.\n'
                           'Action items:\n'
                           '-Ensure that a %s group exists for the specific seller in: Amazon → Configuration → '
                           'FBA Stock Adjustment.\n'
                           '-Set the proper group and Reprocess the file from: Amazon → FBA → Stock Adjustment.') % (
                              code.name, code.name)
                common_log_line_obj.create_common_log_line_ept(
                    message=message, model_name=AMAZON_STOCK_ADJUSTMENT_REPORT_HISTORY, mismatch_details=True,
                    module='amazon_ept', operation_type='import', res_id=self.id,
            ...
            if not config.is_send_email and not config.location_id and not config.group_id.id == self.env.ref(
                ...
                if not config.location_id:
                    message = ('Location is not configured for the adjustment group %s with system ERP ID: %d in '
                               'FBA Stock Adjustment.\n'
                               'Action items:\n'
                               '-Verify the group with ERP ID %d under: Amazon → Configuration → FBA Stock Adjustment.\n'
                               '-Configure the Location field.\n'
                               '-Reprocess the file from: Amazon → FBA → Stock Adjustment.') % (config.group_id.name, config.id, config.id)
                    common_log_line_obj.create_common_log_line_ept(
                        message=message, model_name=AMAZON_STOCK_ADJUSTMENT_REPORT_HISTORY, mismatch_details=True,
                        module='amazon_ept', operation_type='import', res_id=self.id,
                        amz_seller_ept=self.seller_id and self.seller_id.id or False)
    ...
    def process_counter_part_lines(self, config, lines, partially_processed):
        ...
        for line in lines:
            ...
            args = {'line': line, 'temp_lines': temp_lines, 'counter_part_code': code.counter_part_id.name,
                    'reason': reason}
            counter_line_list, remaining_line = self._prepare_counter_line_list(transaction_item_ids,
                                                                                counter_line_list, args, remaining_line)
        ...
        if remaining_line:
            self.with_context(is_remaining_lines_ept=True).process_non_counter_part_lines(
                config, remaining_line, partially_processed)
        return partially_processed
    ...
    def _amz_process_counter_line_list_ept(self, counter_line_list, code_dict, reason_codes):
        ...
        common_log_line_obj = self.env['common.log.lines.ept']
        stock_move_obj = self.env[STOCK_MOVE]
        warhouse_obj = self.env['stock.warehouse']
        ...
        for counter_line in counter_line_list:
            ...
            if counter_vals.get('fulfillment_center_id', '') not in fulfillment_warehouse:
                ...
                if not fn_warehouse or ((counter_vals.get('p_line_disposition', '') != 'SELLABLE' or
                    ...
                    if not fn_warehouse:
                        fulfillment_center = counter_vals.get('fulfillment_center_id', False)
                        message = ('Warehouse not found for fulfillment center %s while processing product: %s.\n'
                                   'Action items:\n'
                                   '-Go to: Amazon → Configuration → Fulfillment Centers.\n'
                                   '-Create or update fulfillment center %s and assign the correct warehouse.\n'
                                   '-Reprocess the file from: Amazon → FBA → Stock Adjustment.') % (
                            fulfillment_center, line.get('MSKU', ''), fulfillment_center)
                    else:
                        message = ('Unsellable Location not set for warehouse: %s while processing product: %s.\n'
                                   'Action items:\n'
                                   '-Go to: Inventory → Configuration → Warehouses.\n'
                                   '-Open warehouse %s and configure the Unsellable Location.\n'
                                   '-Reprocess the file from: Amazon → FBA → Stock Adjustment.') % (
                            fn_warehouse.name, line.get('MSKU', ''), fn_warehouse.name)
                    common_log_line_obj.create_common_log_line_ept(
    ...
    def process_non_counter_part_lines(self, config, lines, partially_processed):
        ...
        for line in lines:
            ...
            if exist_move:
                ...
                vals = self._amz_adjust_prepare_stock_move_vals_ept(product, counter_vals)
                # mark move as Unidentified counterpart? for stock move if counterpart not founded in the odoo
                if self._context.get('is_remaining_lines_ept', False):
                    vals.update({'is_unidentified_counterpart': True})
                stock_move = stock_move_obj.create(vals)
                stock_move_ids.append(stock_move.id)
        ...
        if not warehouse or (line.get('Disposition', '') == 'UNSELLABLE' and not warehouse.unsellable_location_id):
            if not warehouse:
                message = ('Warehouse not found for fulfillment center %s while processing product: %s.\n'
                           ...
                           '-Create or update fulfillment center %s and assign the correct warehouse.\n'
                           '-Reprocess the file from: Amazon → FBA → Stock Adjustment.') % (
                    line.get('Fulfillment Center', False), line.get('MSKU', ''), line.get('Fulfillment Center', False))
            else:
                message = ('Unsellable Location not set for warehouse: %s while processing product: %s.\n'
                           'Action items:\n'
                           '-Go to: Inventory → Configuration → Warehouses.\n'
                           '-Open warehouse %s and configure the Unsellable Location.\n'
                           '-Reprocess the file from: Amazon → FBA → Stock Adjustment.') % (warehouse.name, line.get('MSKU', ''), warehouse.name)
            common_log_line_obj.create_common_log_line_ept(
                message='Mismatch: ' + message, mismatch_details=True,
                model_name=AMAZON_STOCK_ADJUSTMENT_REPORT_HISTORY, module='amazon_ept', operation_type='import',
                res_id=self.id, amz_seller_ept=self.seller_id and self.seller_id.id or False)
        ...
        for stock_move_id in stock_move_ids:
            ...
            try:
                ...
                if message.__contains__('You need to supply a Lot/Serial Number for product'):
                    message = ('Skipped processing the for product%s does not have lot and serial number assigned in '
                               'system at source-location %s') % (str(e).split('\n')[-1], stock_move.location_id.name)
                self.env['common.log.lines.ept'].create_common_log_line_ept(
                    message=message, model_name=AMAZON_STOCK_ADJUSTMENT_REPORT_HISTORY, fulfillment_by='FBA',
                    product_id=stock_move.product_id.id if stock_move.product_id else False, module='amazon_ept',
                    operation_type='import', res_id=self.id, amz_seller_ept=self.seller_id and self.seller_id.id or False)
                continue
        return True

    def _prepare_counter_line_list(self, transaction_item_ids, counter_line_list, args, remaining_line):
        """
        This Method represents to prepare a list of counterpart lines.
        :param transaction_item_ids: []
        :param counter_line_list: []
        :param args: {}
        :param: remaining_line: list []
        :return: []
        """
        common_log_line_obj = self.env['common.log.lines.ept']
        line = args.get('line', False)
        flag = 0
        temp_lines = args.get('temp_lines', False)
        for temp_line in temp_lines:
            if temp_line.get('Reason', '') == args.get('counter_part_code', '') and \
                    ...
                    temp_line.get('Reference ID', False) not in transaction_item_ids and line.get('MSKU', '') == temp_line.get('MSKU', ''):
                if line.get('FNSKU', '') == temp_line.get('FNSKU', '') and \
                ...
                # compare flag and temp_lines if matched means we don't have
                # counterpart in the file for line data
                flag += 1
                if flag == len(temp_lines):
                    remaining_line.append(line)
        return counter_line_list, remaining_line
