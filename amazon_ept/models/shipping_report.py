...
class ShippingReportRequestHistory(models.Model):
    ...
    def get_amz_shipment_report_type(self, seller_id):
        ...
        if amz_seller.is_european_region:
            report_type = ReportType.GET_AMAZON_FULFILLED_SHIPMENTS_DATA_INVOICING.value
        elif amz_seller.country_id.code in ['CA', 'US', 'MX', 'BR']:
            report_type = ReportType.GET_AMAZON_FULFILLED_SHIPMENTS_DATA_TAX.value
        else:
            report_type = ReportType.GET_AMAZON_FULFILLED_SHIPMENTS_DATA_GENERAL.value
        return report_type
    ...
    def check_amazon_report_attachment(self):
        ...
        # activate scheduler for process FBA waiting moves
        cron_id = self.env.ref('amazon_ept.ir_cron_process_fba_shipped_orders_waiting_moves',
                               raise_if_not_found=False)
        if cron_id and not cron_id.sudo().active:
            cron_id.sudo().write({'active': True, 'nextcall': datetime.now() + timedelta(minutes=15)})
    ...
    def check_amz_instance_and_shipment(self, row, instance, order_dict):
        ...
        if not instance:
            message = ('System has not imported the Amazon order ref: %s as Sales Channel: %s is not found in the Odoo.\n'
                       "Action items:\n"
                       "-Verify that the Sales Channel %s is created and active in Odoo.\n"
                       "-If not available, create the Sales Channel with the appropriate name before reimporting the order.") % (
                row.get('amazon-order-id'), row.get('sales-channel'), row.get('sales-channel'))
            ...
            log_line_obj.create_common_log_line_ept(
                ...
                instance.seller_id.id or False, amz_instance_ept=False)
    ...
    def get_amazon_fulfillment_center_warehouse(self, instance, row, fulfillment_warehouse, skip_orders):
        ...
        if fulfillment_id not in fulfillment_warehouse:
            ...
            if not fn_warehouse:
                ...
                message = ('Skipped Amazon order %s because Amazon Fulfillment Center %s not found in Odoo\n'
                           "Action Items:\n"
                           '-Ensure the Fulfillment Center "%s" is defined in Odoo under Amazon -> Configuration -> Setting -> Fulfillment Centers.\n'
                           '-If missing, create the required Fulfillment Center and retry the order import.') % (
                    row.get('amazon-order-id', False), row.get('fulfillment-center-id', False), row.get('fulfillment-center-id', False))
    ...
    def process_amazon_shipment_orders(self, outbound_orders_dict, order_details_dict_list, sale_order_list):
        ...
        report_state = self.amz_get_shipment_report_state_ept(self)
    ...
    def process_shipment_file(self):
        ...
        if self.amz_check_report_required_headers('amazon-order-id', reader.fieldnames,
    ...
    def validate_stock_move(self, move, order_name, process_invoice):
        ...
        try:
            ...
            shipment_report_id = self.id if self else move.amz_shipment_report_id.id
            if self._context.get('is_process_from_scheduler', False):
                is_mismatch_exist = self.amz_get_fba_shipment_mismatch_log(move.id, shipment_report_id)
            if not is_mismatch_exist:
                log_line_obj.create_common_log_line_ept(
                    message=message, model_name=AMZ_SHIPPING_REPORT_REQUEST_HISTORY, fulfillment_by='FBA',
                    module='amazon_ept', operation_type='import', res_id=shipment_report_id,
                    mismatch_details=True, amz_seller_ept=move.seller_id and move.seller_id.id or False,
                    amz_instance_ept=move.amazon_instance_id and move.amazon_instance_id.id or False,
                    stock_move_id=move.id)
        return process_invoice

    def amz_get_fba_shipment_mismatch_log(self, move_id, shipment_report_id):
        """
        Define this method for get created mismatch details log for the move.
        :param: move_id: stock.move() id
        :param: shipment_report_id: shipping.report.request.history() id
        :return: common.log.lines.ept()
        """
        log_line_obj = self.env[COMMON_LOG_LINES_EPT]
        return log_line_obj.search([('res_id', '=', shipment_report_id), ('mismatch_details', '=', True),
    ...
    def amz_create_and_process_fba_invoices(self, order, process_invoice):
        ...
        :return: boolean
        """
        fba_auto_workflow_id = order.amz_instance_id.fba_marketplace_workflow_id if (
            order.amz_instance_id.is_marketplace_workflow) else order.amz_seller_id.fba_auto_workflow_id
    ...
    def prepare_outbound_product_dict_ept(self, amazon_order, ship_line, prod_dict):
        ...
        if not amazon_product:
            message = ("System tried to find an Odoo product with Seller SKU %s but did not find any matching record.\n"
                       "Action Items:\n"
                       "-Check the system configuration for Auto-create Odoo Product.\n"
                       "-Alternatively, manually create a product in Odoo with Internal Reference %s.\n"
                       "-Reimport or reprocess the shipping report.") % (product_sku, product_sku)
            common_log_line_obj.create_common_log_line_ept(
                message=message, model_name=AMZ_SHIPPING_REPORT_REQUEST_HISTORY,
    ...
    def complete_outbound_orders(self, outbound_order_data):
        ...
        for order_ref, lines in outbound_order_data.items():
            ...
            if all([p.state in ('done', 'cancel') for p in order.picking_ids]) and order.invoice_status not in ('invoiced', 'no'):
    ...
    def amz_shipment_get_set_product_ept(self, product):
        ...
        :return:
        """
        if self._context.get('is_fba_live_stock_report_company', False):
    ...
    def check_odoo_product_for_import_order(self, row, instance, order_ref, odoo_product, seller_sku, skip_order):
        ...
        # skip_order = False
        if odoo_product:
            message = ('Odoo product already exist in the system with Amazon product ref: %s with '
                       'Amazon instance: %s') % (seller_sku, instance.name)
            common_log_line_obj.create_common_log_line_ept(
            ...
            skip_order = True
            message = ('System could not find a matching Odoo product for Seller SKU %s, so skipped Amazon order ref: %s.\n'
                       'Action Items:\n'
                       ...
                       '-Alternatively, manually create the product in Odoo with Internal Reference %s and reimport the order.'
                       ) % (seller_sku, order_ref, seller_sku)
            common_log_line_obj.create_common_log_line_ept(
            ...
            message = 'Amazon product was successfully created with Seller Reference %s for instance %s.' % (
        ...
        for row in lines:
            ...
            if not amazon_product:
                ...
                skip_order, odoo_product = self.check_odoo_product_for_import_order(row, instance, order_ref,
                                                                                    odoo_product, seller_sku, skip_order)
    ...
    def auto_process_shipment_report(self, args={}):
        ...
        if seller_id:
            ...
            for report in ship_reports:
                report_type = '' if report.report_type == ReportType.GET_AMAZON_FULFILLED_SHIPMENTS_DATA_GENERAL.value else \
    ...
    def amz_process_fba_waiting_moves_ept(self):
        """
        Define this method for process FBA shipped orders waiting moves.
        :return: True
        """
        ...
        for move in waiting_moves:
            ...
            move._action_assign()
            move.picked = True
            process_invoice = self.with_context(is_process_from_scheduler=True).validate_stock_move(
                move, move.amazon_order_reference, process_invoice)
            if process_invoice:
                # delete created mismatch logs after validating waiting move
                is_mismatch_exist = self.amz_get_fba_shipment_mismatch_log(
                    move.id, move.amz_shipment_report_id.id)
                if is_mismatch_exist:
                    is_mismatch_exist.unlink()
                if move.amz_shipment_report_id.id not in processed_reports:
                    processed_reports.append(move.amz_shipment_report_id.id)
            wrong_moves = stock_move_obj.search([('amazon_instance_id', '=', move.amazon_instance_id.id),
                                                 ('amazon_order_reference', '=', move.amazon_order_reference),
                                                 ('state', 'not in', ['done', 'cancel'])])
            if not wrong_moves:
                invoiced_qty = sum(move.sale_line_id.order_id.invoice_ids.mapped('invoice_line_ids').mapped(
                    'quantity'))
                sale_qty = sum(move.sale_line_id.order_id.order_line.mapped('product_uom_qty'))
                if invoiced_qty < sale_qty:
                    self.amz_create_and_process_fba_invoices(move.sale_line_id.order_id, process_invoice)
            if move_count == 10:
                self._cr.commit()
                move_count = 0
            move_count += 1
        self.amz_update_fba_shipment_report_state(processed_reports)
        return True

    def amz_update_fba_shipment_report_state(self, processed_reports):
        """
        Define this method for update shipment report state after validating stock moves.
        :param: processed_reports: list of processed shipment reports
        :return: True
        """
        for report_id in processed_reports:
            report = self.browse(report_id)
            report_state = self.amz_get_shipment_report_state_ept(report)
            report.write({'state': report_state})
            self._cr.commit()
        return True

    def amz_get_shipment_report_state_ept(self, report):
        """
        Define this method for get shipment report state as per processed report.
        :param: report: shipping.report.request.history() id
        :return: report state - str
        """
        stock_move_obj = self.env[STOCK_MOVE]
        log_line_obj = self.env[COMMON_LOG_LINES_EPT]
        model_id = self.env[IR_MODEL]._get(AMZ_SHIPPING_REPORT_REQUEST_HISTORY).id
        is_partially_processed_report = stock_move_obj.search_count([
            ('amz_shipment_report_id', '=', report.id), ('state', 'not in', ('done', 'cancel'))])
        mismatch_logs = log_line_obj.search_count([('res_id', '=', report.id), ('model_id', '=', model_id),
                                                   ('mismatch_details', '=', True)])
        return 'partially_processed' if is_partially_processed_report or mismatch_logs else 'processed'
