...
class RatingReportHistory(models.Model):
    ...
    def default_get(self, fields):
        ...
        if not fields:
            return res
        res.update({'report_type': ReportType.GET_SELLER_FEEDBACK_DATA.value})
        return res
    ...
    def auto_import_rating_report(self, args={}):
        ...
        if seller_id:
            ...
            report_type = ReportType.GET_SELLER_FEEDBACK_DATA.value
    ...
    def process_rating_report(self):
        ...
        for row in reader:
            ...
            if not amazon_sale_order:
                message = ('System tried to search order with Amazon order ref %s but did not find it in Odoo.\n'
                           "Action Items:\n"
                           "-Check the order in Amazon, and based on that, import the order using the Operation Wizard.") % amz_order_id
                common_log_line_obj.create_common_log_line_ept(
                    message=message, model_name='rating.report.history', module='amazon_ept', operation_type='import',
            ...
            if not amazon_order_rating:
                ...
                message = 'Rating already exists for Amazon order reference: %s.' % amz_order_id
                common_log_line_obj.create_common_log_line_ept(
                    message=message, model_name='rating.report.history', module='amazon_ept', operation_type='import',
