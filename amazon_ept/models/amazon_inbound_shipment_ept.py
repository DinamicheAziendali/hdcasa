...
class InboundShipmentEpt(models.Model):
    ...
    def amz_find_location_routes_ept(self, warehouse):
        ...
        if not location_routes:
            error_value = ('Cannot generate transfer, routes not found for warehouse: %s (Shipment: %s).\n'
                           ...
                           '-Update routing configuration and re-import the shipment.') % (warehouse.name,
                                                                                           self.name)
            ...
            if shipment_status in ['RECEIVING', 'CLOSED']:
                ...
                if pickings:
                    ...
                    if backorders:
                        ...
                        backorders.write({'old_shipment_pick_done_by': 'by_user'})
                    ...
                    if odoo_shipment_rec:
                        ...
                        if pickings:
                            pickings = self.with_context(
                                amz_inb_manually_process_ept=True).amz_cancel_waiting_state_pickings(pickings, odoo_shipment_rec)
                    if not pickings:
                        flag = False
                ...
                if shipment_status == 'CLOSED':
                    ...
                    if pickings:
                        ...
                        pickings.write({'old_shipment_pick_done_by': 'by_user'})
    ...
    def amz_cancel_waiting_state_pickings(self, pickings, odoo_shipment_rec):
        """
        ...
        for picking in pickings.filtered(lambda pick: pick.state in ('waiting', 'confirmed')):
            if not picking.move_ids.filtered(lambda move: move.product_id.tracking in ('serial', 'lot')):
                ...
                if self._context.get('amz_inb_manually_process_ept', False):
                    process_type = 'by_user'
                else:
                    process_type = 'by_scheduler'
                picking.write({'old_shipment_pick_done_by': process_type})
    ...
    def amz_prepare_picking_vals_ept(picking):
        ...
        return {
            ...
            'old_shipment_pick_done_by': 'by_user'
        }
    ...
    def amz_create_back_orders_and_check_return_picking(self, shipment, shipmentid, amazon_shipment):
        ...
        if backorders:
            backorders.action_assign()
            backorders.write({'old_shipment_pick_done_by': 'by_scheduler'})
    ...
    def get_remaining_qty_ept(self, instance, amazon_shipment_id, odoo_shipment_rec, items):
        ...
        if picking:
            for item in items:
                ...
                if not amazon_product:
                    message = ("Product not found for Seller SKU: %s & Fulfillment Network SKU: %s "
                               "during inbound shipment import.\n"
                               "Action items:\n"
                               "-Create a new product in Odoo with reference: %s, OR\n"
                               "-Map this SKU to an existing product and re-import the shipment.") % (sku, asin, sku)
                    log_line_obj.create_common_log_line_ept(
                        message=message, model_name=AMZ_INBOUND_SHIPMENT_EPT, module='amazon_ept',
    ...
    def get_old_shipment_report_data(self):
        """
        ...
        instance = self.get_instance(self)
        kwargs = self.amz_prepare_inbound_shipment_kwargs_vals(instance)
        kwargs.update({'emipro_api': 'check_status_by_shipment_ids', 'shipment_ids': [str(self.shipment_id)]})
        response = iap_tools.iap_jsonrpc(DEFAULT_ENDPOINT, params=kwargs, timeout=1000)
        if response.get('error', {}):
            raise UserError(_(response.get('error', {})))
