...
LAST_SYNC_FIELDS = ['removal_order_report_last_sync_on', 'shipping_report_last_sync_on',
                    ...
                    'update_shipment_last_sync_on', 'cancel_fbm_order_last_sync_on', 'last_new_inbound_shipment_status_sync',
                    'replacement_orders_report_last_sync_on']
...
class AmazonSellerEpt(models.Model):
    ...
    last_new_inbound_shipment_status_sync = fields.Datetime()
    ...
    is_create_schedule_activity_for_outbound = fields.Boolean("Create Schedule Activity for failed order? ", default=False,
                                                     help="If checked, Then Schedule Activity will create for failed "
                                                          "outbound order.")
    amz_outbound_activity_user_ids = fields.Many2many('res.users', 'res_users_amazon_outbound_seller_rel',
                                             'amz_seller_id', 'res_users_id', string='Responsible User (Outbound)')
    ...
    amz_outbound_activity_date_deadline = fields.Integer(string='Deadline lead days (Outbound)',
                                                         help="its add number of days in schedule activity deadline date ")
    replacement_order_report_days = fields.Integer("Replacement Orders Report Days", default=3,
                                                   help="Days of report to import Replacement Orders Report")
    auto_create_fba_repl_orders_report = fields.Boolean(string="Auto Create FBA Replacement Orders Report ?")
    replacement_orders_report_last_sync_on = fields.Datetime("Last Replacement Orders Report Request Time")
    account_journal_payment_credit_account_id = fields.Many2one(
        comodel_name='account.account', string='Outstanding Payments', readonly=False, check_company=True,
        domain="[('deprecated', '=', False), ('account_type', '=', 'asset_current')]",
        help='If this configuration is enabled, the system will reconcile Amazon transactions using this account '
             'if no reconcile account is specified in the Amazon transaction configuration.')
    amz_auto_import_awd_shipment = fields.Boolean(string='Auto Import AWD Inbound Shipment Report ?')
    amz_last_auto_import_awd_shipment_date = fields.Datetime()
    ...
    def unarchive_seller(self):
        ...
        if fba_warehouse_ids:
            ...
            if len(location) == 1:
                self._cr.execute('update stock_location set active=True where id =%s', (location.id,))
            else:
    ...
    def amz_create_analytic_account_for_seller(self, amazon_sellers):
        ...
        for amazon_seller in amazon_sellers:
            if not amazon_seller.amz_seller_analytic_account_id:
                ...
                # Skipping  analytic account ID setup during seller creation as per discussion
    ...
    def amz_create_analytic_account_for_marketplaces(self, amazon_marketplaces):
        ...
        for amazon_marketplace in amazon_marketplaces:
            if not amazon_marketplace.analytic_account_id:
                ...
                # Skipping marketplace analytic account ID setup during instance creation as per discussion
        ...
        if seller_id:
            ...
            if amazon_inbound_shipments:
                ...
                while max_range < len(ship_id_list):
                    ...
                    inbound_shipments = {}
                    shipment_status = {}
                    instance_ids = seller.instance_ids.ids
                    for item in items:
                        shipment_id = item.get('ShipmentId', '')
                        inbound_shipment_id = inbound_shipment_obj.search(
                            [('shipment_id', '=', shipment_id), ('instance_id_ept', 'in', instance_ids)])
                        if inbound_shipment_id and inbound_shipment_id.state != 'CLOSED':
                            inbound_shipments.update({(shipment_id, item.get('ShipmentStatus')):
                                                          amazon_inbound_shipments.get(shipment_id)})
                    self.prepare_inbound_shipment_report_data(inbound_shipments, seller.id, 'old')
                    ...
                    for inbound_shipment in inbound_shipments:
                        ...
                        if ship_status == 'CLOSED':
                            ...
                            pickings and pickings.write({'old_shipment_pick_done_by': 'by_scheduler'})
    ...
    @api.model
    def auto_import_new_shipment_status_ept(self, args={}):
        """
        Define this method for auto check status of new inbound shipment.
        :return: True
        """
        ...
        if seller_id:
            ...
            current_date = datetime.now(timezone.utc)
            last_updated_before = current_date
            if seller.last_new_inbound_shipment_status_sync:
                last_sync_time = seller.last_new_inbound_shipment_status_sync
            else:
                last_sync_time = datetime.now(timezone.utc)
            last_updated_after = (last_sync_time - timedelta(days=1))
            # preparing request kwargs
            ...
            for amazon_result in items:
                ...
                if amazon_inbound_shipments:
                    ...
                    amazon_inbound_shipments.update({shipment_id: [amazon_result]})
            if amazon_inbound_shipments:
                inbound_shipment_obj.check_status_ept_v2024(amazon_inbound_shipments, seller)
                max_range = 0
                ship_id_list = amazon_inbound_shipments.keys()
                while max_range < len(ship_id_list):
                    ship_id_list = list(ship_id_list)
                    ...
                    for item in items:
                        shipment_id = item.get('ShipmentId', '')
                        inbound_shipment_id = self.env['inbound.shipment.new.ept'].search(
                            [('shipment_confirmation_id', '=', shipment_id), ('instance_id_ept', 'in', instance_ids)])
                        if inbound_shipment_id and inbound_shipment_id.state != 'CLOSED':
                            inbound_shipments.update({(shipment_id, item.get('ShipmentStatus')):
                                                          amazon_inbound_shipments.get(shipment_id)})
                    if inbound_shipments:
                        self.prepare_inbound_shipment_report_data(inbound_shipments, seller.id, 'new')
                    for ship_member in items:
                        shipmentid = ship_member.get('ShipmentId', '')
                        status = ship_member.get('ShipmentStatus', '')
                        shipment_status[shipmentid] = status
                    inbound_shipment_ids = shipment_status.keys()
                    inbound_shipments = inbound_shipment_obj.search(
                        [('shipment_confirmation_id', 'in', list(inbound_shipment_ids))])
                    for inbound_shipment in inbound_shipments:
                        ship_status = shipment_status.get(inbound_shipment.shipment_confirmation_id)
                        inbound_shipment.write({'state': ship_status})
                        if ship_status == 'CLOSED':
                            if not inbound_shipment.closed_date:
                                inbound_shipment.write({'closed_date': time.strftime("%Y-%m-%d")})
                            pickings = inbound_shipment.mapped('picking_ids').filtered(
                                lambda r: r.state not in ['done', 'cancel'] and r.is_fba_wh_picking)
                            pickings and pickings.action_cancel()
                            pickings and pickings.write({'shipment_pick_done_by': 'by_scheduler'})
                            # create qty difference return for inbound shipment
                            self.amz_return_shipment_qty_difference_v2024(inbound_shipment)
                message = "%s inbound shipment is successfully processed" % (len(amazon_inbound_shipments))
                common_log_line_obj.create_common_log_line_ept(message=message,
                                                               model_name='inbound.shipment.new.ept',
                                                               module='amazon_ept', operation_type='import',
                                                               amz_seller_ept=seller.id)
            seller.last_new_inbound_shipment_status_sync = last_updated_before
        return True
    ...
    @api.model
    def auto_all_check_status_new_inbound_shipment(self, args={}):
        """
        Define this method for auto check status of all inbound shipment which check status
        are not done via main scheduler of check status.
        :param args: dict {}
        ...
        if seller_id:
            ...
            for shipment in shipments:
                try:
                    ...
                    common_log_line_obj.create_common_log_line_ept(
                        message=str(ex), model_name='inbound.shipment.new.ept',
                        module='amazon_ept', operation_type='import', res_id=shipment.id,
                        order_ref=shipment.shipment_confirmation_id, amz_seller_ept=seller_id)
                self._cr.commit()
        return True
    ...
    def amz_return_shipment_qty_difference_v2024(self, inbound_shipment):
        """
        Define this method for create return picking for qty difference for shipment.
        :param: inbound_shipment : amazon.inbound.shipment.ept()
        :return: True
        ...
        inbound_shipment_obj = self.env['inbound.shipment.new.ept']
        stock_picking_obj = self.env['stock.picking']
        instance = inbound_shipment.get_instance()
        kwargs = inbound_shipment.amz_prepare_inbound_shipment_kwargs_vals(instance)
        kwargs.update({'emipro_api': 'get_shipment_items_by_shipment_sp_api_v2024',
                       'amazon_shipment_id': inbound_shipment.shipment_confirmation_id})
        response = iap_tools.iap_jsonrpc(DEFAULT_ENDPOINT, params=kwargs, timeout=1000)
        if response.get('error', False):
            common_log_line_obj.create_common_log_line_ept(
                message=response.get('error', ''), model_name='inbound.shipment.new.ept',
                module='amazon_ept', operation_type='import',
                amz_seller_ept=instance.seller_id and instance.seller_id.id or False,
                amz_instance_ept=instance and instance.id or False)
        else:
            inbound_shipment_obj.get_remaining_qty_ept_v2024(
                inbound_shipment.instance_id_ept, inbound_shipment.shipment_confirmation_id, inbound_shipment,
                response.get('result', {}).get('ItemData', {}))
            stock_picking_obj.check_qty_difference_and_create_return_picking_ept_v2024(
                inbound_shipment.shipment_confirmation_id, inbound_shipment, inbound_shipment.instance_id_ept,
                response.get('result', {}).get('ItemData', {}))
        return True
    ...
    def prepare_inbound_shipment_report_data(self, amazon_shipments, seller_id, shipment_model_type):
        """
        Prepare the inbound shipment report data and triggers other methods to create the report.
        :param: amazon_shipments {}
        :param: seller_id amazon.seller.ept()->id
        :param: shipment_model_type : str, 'new' or 'old' for the type of shipment model.
        """
        inbound_ship_report_data = []
        for shipment, item_details in amazon_shipments.items():
            shipment_id = shipment[0]
            current_status = shipment[1]
            items_data = []
            for item in item_details:
                seller_sku = item.get('SellerSKU', '')
                qty_received = item.get('QuantityReceived', 0)
                qty_shipped = item.get('QuantityShipped', 0)
                items_data.append({'seller_sku': seller_sku, 'qty_received': qty_received,'qty_shipped': qty_shipped})
            inbound_ship_report_data.append({'shipment_id': shipment_id, 'items': items_data, 'status': current_status})
        if inbound_ship_report_data:
            self.env['inbound.shipment.report.data.ept'].create_shipment_report_data(inbound_ship_report_data,
                                                                                     seller_id, shipment_model_type)
        return True
