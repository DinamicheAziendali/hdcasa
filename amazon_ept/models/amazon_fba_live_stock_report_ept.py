...
class AmazonLiveStockReportEpt(models.Model):
    ...
    def list_of_inventory(self):
        ...
        action = {
            'domain': [('reference', 'ilike', f'{self.name}%')],
            'name': 'FBA Live Stock Inventory',
            'view_mode': 'list,form',
            'res_model': 'stock.move',
            'type': 'ir.actions.act_window',
            'views': [(stock_move_tree_view.id if stock_move_tree_view else False, 'list'),
    ...
    def _compute_inventory_count(self):
        ...
        for record in self:
            record.inventory_count = len(stock_move.search([('reference', 'ilike', f'{self.name}%')]))
    ...
    def auto_import_amazon_fba_live_stock_report(self, args={}):
        ...
        if seller:
            ...
            vals = {'seller_id': seller.id, 'report_type': ReportType.GET_FBA_MYI_UNSUPPRESSED_INVENTORY_DATA.value}
    ...
    def get_inventory_report(self, vals):
        ...
        if inv_report_ids:
            ...
            if len(inv_report_ids) > 1:
                ...
                result['res_id'] = inv_report_ids[0] if inv_report_ids and len(inv_report_ids) > 0 else False
        return result
    ...
    def process_fba_live_stock_report(self):
        ...
        if not self.attachment_id:
            message = ("Attachment not found in the FBA Inventory Report.\n"
                       "Action items:\n"
                       "-Verify the attachment exists in the report's chatter.\n"
                       "-You can download the file from: Amazon → FBA → Inventory Reports.\n"
                       "-Once attached, proceed with report processing.")
            if not self._context.get('is_auto_process', False):
        ...
        if self.amz_check_report_required_headers('afn-fulfillable-quantity', reader.fieldnames,
    ...
    def fill_dictionary_from_file_by_instance(self, reader):
        ...
        """
        common_log_line_obj = self.env['common.log.lines.ept']
        shipment_report_obj = self.env['shipping.report.request.history']
        sellable_line_dict = {}
        ...
        for row in reader:
            ...
            if not odoo_product:
                odoo_product = self.amazon_find_odoo_product_ept(seller_sku)
                if not odoo_product:
                    message = ("Product not found in Odoo for Seller SKU: %s.\n"
                               "Action items:\n"
                               "-Create a product with internal reference: %s, OR\n"
                               "-Assign it to an existing product and map it to Amazon.\n"
                               "-Re-process the inventory report.") % (seller_sku, seller_sku)
                    common_log_line_obj.create_common_log_line_ept(
                        message=message, model_name=FBA_LIVE_STOCK_REPORT, mismatch_details=True, module='amazon_ept',
                        operation_type='import', res_id=self.id,
                        amz_seller_ept=self.seller_id and self.seller_id.id or False)
                    continue
                if not odoo_product.is_storable:
                    message = "Product %s is not storable product." % odoo_product.name
                    common_log_line_obj.create_common_log_line_ept(
                        message=message, model_name=FBA_LIVE_STOCK_REPORT, mismatch_details=True, module='amazon_ept',
                        operation_type='import', res_id=self.id,
                        amz_seller_ept=self.seller_id and self.seller_id.id or False)
            ...
            if mrp_module:
                bom_lines = shipment_report_obj.with_context(
                    is_fba_live_stock_report_company=self.company_id.id).amz_shipment_get_set_product_ept(odoo_product)
                if bom_lines:
                    for bom_line in bom_lines:
                        odoo_product_id = bom_line[0].product_id.id
                        afn_fulfillable_qty = bom_line[1].get('qty', 0.0) * float(row.get('afn-fulfillable-quantity', 0.0))
                        afn_reserved_qty = bom_line[1].get('qty', 0.0) * float(row.get('afn-reserved-quantity', 0.0))
                        afn_unsellable_qty = bom_line[1].get('qty', 0.0) * float(row.get('afn-unsellable-quantity', 0.0))
                        sellable_line_dict, unsellable_line_dict = self.amz_update_sellable_unsellable_qty_dict(
                            odoo_product_id, sellable_line_dict, unsellable_line_dict, afn_fulfillable_qty,
                            afn_reserved_qty, afn_unsellable_qty)
                else:
                    sellable_line_dict, unsellable_line_dict = self.amz_update_sellable_unsellable_qty_dict(
                        odoo_product_id, sellable_line_dict, unsellable_line_dict, afn_fulfillable_qty,
                        afn_reserved_qty, afn_unsellable_qty)
    ...
    def amazon_find_odoo_product_ept(self, seller_sku):
        """
        Define this method to find the odoo product based on seller sku.
        :param: seller_sku: str
        :return: product.product()
        """
        product_obj = self.env['product.product']
        return product_obj.search([('default_code', '=', seller_sku)], limit=1)

    def amz_update_sellable_unsellable_qty_dict(self, odoo_product_id, sellable_line_dict, unsellable_line_dict,
                                                afn_fulfillable_qty, afn_reserved_qty, afn_unsellable_qty):
        """
        Define this method for update sellable and unsellable qty dict.
        :param: odoo_product_id: product.product() id
        :param: sellable_line_dict: dict {}
        :param: unsellable_line_dict: dict {}
        :param: afn_fulfillable_qty: flot
        :param: afn_reserved_qty: float
        :param: afn_unsellable_qty:
        :return: dict {}, dict {}
        """
    ...
    def create_stock_inventory_from_amazon_live_report(self, sellable_line_dict, unsellable_line_dict, amz_warehouse):
        ...
        if sellable_line_dict:
            ...
            quant_list_sellable = stock_quant.with_context(
                fba_live_inv_id=self.id).with_company(self.seller_id.company_id.id).create_inventory_adjustment_ept(
        ...
        if not amz_warehouse.unsellable_location_id:
            message = ('Unsellable location not set for warehouse: %s.\n'
                       'Action items:\n'
                       '-Go to: Inventory → Configuration → Warehouses.\n'
                       '-Open the warehouse and configure the Unsellable Location.\n'
                       '-Re-process the file from: Amazon → FBA → Inventory Reports.') % amz_warehouse.name
            common_log_line_obj.create_common_log_line_ept(
                message=message, model_name=FBA_LIVE_STOCK_REPORT, mismatch_details=True, module='amazon_ept',
