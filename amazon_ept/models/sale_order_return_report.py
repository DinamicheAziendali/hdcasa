...
class SaleOrderReturnReport(models.Model):
    ...
    def _compute_total_returns(self):
        ...
        picking_records = stock_move_obj.search([('amz_return_report_id', '=', self.id),
                                                 ('picking_id', '!=', False)]).mapped('picking_id')
        self.return_count = len(records)
        self.return_picking_count = len(picking_records)
    ...
    def list_of_return_pickings(self):
        """
        List all Picking which is returned in the current report
        :return:
        """
        stock_move_obj = self.env[STOCK_MOVE]
        picking_records = stock_move_obj.search([('amz_return_report_id', '=', self.id),
                                                 ('picking_id', '!=', False)]).mapped('picking_id')
        action = {
            'domain': "[('id', 'in', " + str(picking_records.ids) + " )]",
            'name': 'Amazon FBA Order Return Picking',
            'view_mode': 'list,form',
    ...
    def default_get(self, fields):
        ...
        if not fields:
            return res
        res.update({'report_type': ReportType.GET_FBA_FULFILLMENT_CUSTOMER_RETURNS_DATA.value})
        return res
    ...
    def check_amz_return_move_line_ept(self, line, fulfillment_warehouse):
        ...
        return_date = parser.parse(line.get('return-date', '')).date()
        ...
        if not amazon_orders:
            message = (
                          'Amazon Order ref: %s not found in Odoo. With Return date : %s, Order-id : %s, SKU : %s, Quantity : %s, Detailed-Disposition : %s\n'
                          'Action items:\n'
                          '-The order must exist in Odoo to create a return.\n'
                          '-You can import it using the operation wizard and reprocess the return report.') % (
                          amazon_order_id, return_date, amazon_order_id, sku, returned_qty, disposition)
        ...
        if not amazon_products:
            message = ('Product: %s not found for Amazon Order ref: %s.\n'
                       'Action items:\n'
                       '-Create a product with internal reference: %s, OR\n'
                       '-Assign the reference to an existing product and map it to Amazon.\n'
                       '-Re-process the file from: Amazon → FBA → Customer Return Reports.') % (sku, amazon_order_id, sku)
            log_line_obj.create_common_log_line_ept(
                message=message, model_name=SALE_ORDER_RETURN_REPORT, module='amazon_ept', operation_type='import',
                res_id=self.id, mismatch_details=True, amz_seller_ept=self.seller_id and self.seller_id.id or False)
            return True, [], fulfillment_warehouse
        ...
        if not amazon_order_lines:
            message = ('Sale order line not found for SKU: %s in Amazon order(s): [%s].\n'
                       'Action items:\n'
                       '-Ensure the sale order line with this product exists for the given orders.\n'
                       '-If not available, the entry can be skipped.') % (sku, amazon_order_id)
            log_line_obj.create_common_log_line_ept(
                message=message, model_name=SALE_ORDER_RETURN_REPORT, module='amazon_ept', operation_type='import',
                res_id=self.id, mismatch_details=True, amz_seller_ept=self.seller_id and self.seller_id.id or False)
            return True, [], fulfillment_warehouse
        ...
        if not warehouse:
            message = ('Warehouse not found for Fulfillment Center: %s to process customer return for Order ref: %s.\n'
                       'Action items:\n'
                       '-Ensure Fulfillment Center %s is configured under: Amazon → Configuration → Fulfillment Centers.\n'
                       '-Assign a proper warehouse and reprocess the customer return report.') % (fulfillment_center_id, amazon_order_id, fulfillment_center_id)
            log_line_obj.create_common_log_line_ept(
                message=message, model_name=SALE_ORDER_RETURN_REPORT, module='amazon_ept', operation_type='import',
                res_id=self.id, mismatch_details=True, amz_seller_ept=self.seller_id and self.seller_id.id or False)
            return True, [], fulfillment_warehouse
    ...
    def process_return_report_file(self):
        ...
        for line in reader:
            ...
            if not already_processed:
                product_id = amazon_order_lines[0].product_id.id
                customer_moves = move_lines.filtered(lambda move:not move.fba_returned_date and move.product_id.id == product_id)
                return_moves = move_lines.filtered(lambda move:move.fba_returned_date and move.product_id.id == product_id)
                cust_prd_qty = sum(customer_moves.mapped('product_qty'))
                return_prd_qty = sum(return_moves.mapped('product_qty'))
                qty_diff = cust_prd_qty - return_prd_qty
                if qty_diff <= 0:
                    already_processed = True
            if already_processed:
                message = 'System already processed return for order: %s.' % amazon_order_id
                log_line_obj.create_common_log_line_ept(message=message, model_name=SALE_ORDER_RETURN_REPORT,
            ...
            if not move_lines:
                ...
                if move_lines:
                    return_move_dict, remaning_move_qty, return_picking_dict = self.process_kit_type_product(
                        ...
                        remaning_move_qty, return_picking_dict)
                    continue
            if not move_lines:
                message = ('Cannot process customer return for Amazon Order ref: %s — delivery stock moves not found or '
                           'already returned.\n'
                           'Action items:\n'
                           '-Manually verify stock moves in this order.\n'
                           '-Ensure delivery stock moves exist and are not already returned.') % amazon_order_id
                log_line_obj.create_common_log_line_ept(
                    message=message, model_name=SALE_ORDER_RETURN_REPORT, module='amazon_ept', operation_type='import',
                    res_id=self.id, mismatch_details=True, amz_seller_ept=self.seller_id and self.seller_id.id or False)
                continue
            return_move_dict, remaning_move_qty, return_picking_dict = self.get_required_moves_for_return_process(
                move_lines, warehouse, return_date, sku, disposition, reason, status, fulfillment_center_id,
                remaning_move_qty, return_move_dict, amazon_order_id, returned_qty, return_picking_dict)
        if return_picking_dict:
            self._create_fba_returns_picking(return_picking_dict)
        if return_move_dict:
    ...
    def get_required_moves_for_return_process(self, move_lines, warehouse, return_date, sku, disposition,
                                              ...
                                              return_move_dict, amazon_order_id, returned_qty, return_picking_dict):
        """
        ...
        for move in move_lines:
            ...
            if round(qty, 2) <= 0:
                message = ('No stock found from quant history for Amazon Order ref: %s.\n'
                           'Action items:\n'
                           '-Check delivery stock moves for the order.\n'
                           "-Ensure the delivered quantity exists and hasn't already been returned.\n"
                           "-Make manual adjustments if required.") % (amazon_order_id)
                log_line_obj.create_common_log_line_ept(
                    message=message, model_name=SALE_ORDER_RETURN_REPORT, module='amazon_ept', operation_type='import',
                    res_id=self.id, mismatch_details=True, amz_seller_ept=self.seller_id and self.seller_id.id or False)
                continue
            ...
            if move.picking_id:
                key = (move, warehouse, return_date, sku, disposition, reason, status,
                       fulfillment_center_id, move.picking_id)
            else:
                key = (move, warehouse, return_date, sku, disposition, reason, status,
            ...
            if move.picking_id:
                if move.picking_id not in return_picking_dict:
                    return_picking_dict.update({move.picking_id: {key: new_return_move_qty}})
                else:
                    qty = return_picking_dict.get(move.picking_id, {}).get(key, 0.0)
                    return_picking_dict[move.picking_id][key] = qty + new_return_move_qty
            else:
            ...
            if returned_qty <= 0:
                break
        return return_move_dict, remaning_move_qty, return_picking_dict
    ...
    def _create_fba_returns_picking(self, return_picking_dict):
        """
        Process Stock Picking from return_picking_dict and create returns picking and their correspond stock moves
        in the same warehouse where shipped or in the reimbursement or in unsellable locations
        :param return_picking_dict: {}
        :return: True
        """
        reason_record_dict = {}
        fulfillment_center_dict = {}
        for picking, key_qty in return_picking_dict.items():
            ...
            for key, qty in key_qty.items():
                move, warehouse, return_date, sku, disposition, reason, status, fulfillment_center_id, picking_id = key
                pick_type_id = picking_id.picking_type_id.return_picking_type_id.id if (
                    picking_id.picking_type_id.return_picking_type_id) else picking_id.picking_type_id.id

                location_dest_id, reason_record_dict, fulfillment_center_dict = \
                    self.update_reason_record_and_fulfillment_center_dict(key, reason_record_dict,
                                                                          fulfillment_center_dict)
                if not return_picking:
                    return_picking = picking_id.copy({'move_ids': [], 'picking_type_id': pick_type_id, 'state': 'draft',
                                                      'origin': picking_id.origin,
                                                      'location_id': move.location_dest_id.id,
                                                      'location_dest_id': location_dest_id})
                amz_return_move = move.copy({'product_id': move.product_id.id, 'product_uom_qty': abs(qty),
                                             'picking_id': return_picking.id if return_picking else False,
                                             'state': 'draft', 'location_id': move.location_dest_id.id,
                                             'location_dest_id': location_dest_id, 'picking_type_id': pick_type_id,
        ...
        for move in move_lines:
            ...
            for bom_line, line in one_set_product_dict:
                if bom_line.product_id.id == move.product_id.id:
                    bom_qty = line['qty']
                    break
            if float_is_zero(bom_qty, precision_digits=2):
                continue
            if move.picking_id:
                key = (move, warehouse, return_date, sku, disposition, reason, status,
                       fulfillment_center_id, move.picking_id)
            ...
            if move.picking_id:
                if move.picking_id not in return_picking_dict:
                    return_picking_dict.update({move.picking_id: {key: new_final_ret_qty}})
                else:
                    qty = return_picking_dict.get(move.picking_id, {}).get(key, 0.0)
                    return_picking_dict[move.picking_id][key] = qty + new_final_ret_qty
            else:
                if move not in return_move_dict:
