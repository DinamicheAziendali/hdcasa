...
amazon_messages = {'fba_warehouse_id': "The order import operation failed because the warehouse configuration was not "
                                       "found in the instance configuration.\n"
                                       "Action Items:\n"
                                       "-Navigate to Amazon >> Configuration >> Settings, select Marketplace and "
                                       "configure FBA Warehouse.\n"
                                       "-Reimport order from the operations.",
                   'picking_policy': "The order import operation failed because the shipping policy configuration was "
                                     "not found in the auto invoice workflow configuration.\n"
                                     "Action items:\n"
                                     "-Navigate to Amazon >> Configuration >> Settings.\n"
                                     "-Select Seller and Review whether Auto Workflow is configured, and within Auto "
                                     "workflow, ensure that the shipping policy is also configured.",
                   'ship_product': "When creating a new delivery method, the system encountered an issue as it could not"
                                   " find the shipping product in the instance configuration.\n"
                                   "Action items:\n"
                   ...
                   'fbm_warehouse_id': "The order import operation failed because the warehouse configuration was not"
                                       "found in the instance configuration.\n"
                                       "Action Items:\n"
                                       "-Navigate to Amazon >> Configuration >> Settings\n"
                                       "-Select the marketplace and configure the FBM warehouse.",
                   'pricelist_id': "The order import operation failed because the pricelist configuration was not found "
                                   "in the instance configuration.\n"
                                   "Action items:\n"
                                   "-Navigate to Amazon >> Configuration >> Settings, select Marketplace  and configure"
...
class SaleOrder(models.Model):
    ...
    amz_fba_replaced_order_id = fields.Many2one('sale.order', string='Replaced Order')
    amz_fba_original_order_id = fields.Many2one('sale.order', string='Original Order')
    amz_fba_replaced_order_reason = fields.Char(help='Order Replacement reason provided by the buyer')
    amz_reimburse_invoice_ids = fields.One2many('account.move', 'so_reimburse_invoice_id',
                                                string='Reimbursement Invoice', help='Reimbursement Invoice for Order')
    reimburse_invoice_count = fields.Integer(compute='_compute_reimburse_invoices', readonly=True)

    def _compute_reimburse_invoices(self):
        """
        This method will count the number of reimbursement invoices.
        :return:
        """
        self.reimburse_invoice_count = len(self.amz_reimburse_invoice_ids.ids)

    def action_open_reimbursement_invoices(self):
        """
        This method will return the reimbursement invoice list
        :return:
        """
        invoices = self.amz_reimburse_invoice_ids
    ...
    mismatch_delivered_invoice_qty = fields.Boolean(compute="_compute_mismatch_order_qty",
                                                    search="_search_amazon_order_ids", copy=False)

    def _compute_mismatch_order_qty(self):
        for order in self:
            mismatch_order = False
            if order.amz_instance_id:
                order_lines = order.order_line
                for order_line in order_lines:
                    if order_line.qty_delivered != order_line.qty_invoiced:
                        mismatch_order = True
                        break
                order.mismatch_delivered_invoice_qty = mismatch_order
                continue
            order.mismatch_delivered_invoice_qty = False

    def _search_amazon_order_ids(self, operator, value):
        query = """SELECT so.id FROM sale_order AS so
                           ...
                           WHERE sol.qty_delivered != sol.qty_invoiced 
                           AND so.amz_instance_id IS NOT NULL 
                           AND pt.type = 'product'
                        """
        self._cr.execute(query)
        results = self._cr.fetchall()
        ...
        for amz_order in amazon_orders:
            if getattr(amz_order.partner_id, 'is_amz_customer', False) and amz_order.amz_fulfillment_by == 'FBA' and\
                ...
                for shipment, shipment_item in list(shipment_ids.items()):
                    to_invoice = amz_order.order_line.filtered(lambda l: not float_is_zero(l.qty_to_invoice, precision_digits=2))
                    if to_invoice:
    ...
    def _prepare_invoice(self):
        ...
        if self.amz_instance_id:
            ...
            # if order is Amazon order and fiscal position was not set in the order then
            # do not allow to set the fiscal position in the invoice which set in the invoice vals
            # from the default odoo flow. Here if we set the fiscal_position_id = False in the invoice vals
            # then system will not call the _compute_fiscal_position_id() method of the odoo base and not set
            # the fiscal position in the invoice
            if not self.fiscal_position_id:
                res.update({'fiscal_position_id': False})
        return res
    ...
    def check_and_cancel_amazon_orders(self, seller, fulfillment_by, orders):
        ...
        for order in orders:
            ...
            if not existing_order:
                message = ('System attempted to cancel Amazon order Ref %s but did not find it in the system.\n'
                           "Action Items:\n"
                           "-Confirm whether the order was previously imported.\n"
                           "-If not present, no cancellation is needed in Odoo.") % amazon_order_ref
                log_line_obj.create_common_log_line_ept(message=message, model_name=SALE_ORDER,
                                                        ...
                                                        module='amazon_ept', operation_type='import', res_id=self.id,
                                                        amz_seller_ept=seller and seller.id or False, mismatch_details=False,
            ...
            if existing_order and existing_order.picking_ids.filtered(lambda pick: pick.state == 'done'):
                message = ('System tried to cancel Amazon order Ref %s but found a validated delivery order.\n'
                           "Action Items:\n"
                           "-System can only cancel orders where delivery has not been validated.\n"
                           "-For validated deliveries, manually create a return transfer and cancel the order manually in Odoo.") % amazon_order_ref
                log_line_obj.create_common_log_line_ept(message=message, model_name=SALE_ORDER,
                                                        order_ref=existing_order.name, fulfillment_by=fulfillment_by,
                                                        module='amazon_ept', operation_type='import', res_id=self.id,
                                                        amz_seller_ept=seller and seller.id or False, mismatch_details=True,
                                                        amz_instance_ept=instance and instance.id or False)
                continue
            ...
            for order in orders:
                ...
                if message:
                    common_log_line_obj.create_common_log_line_ept(message=message, model_name=SALE_ORDER,
                                                                   ...
                                                                   module='amazon_ept', operation_type='import',
                                                                   res_id=self.id, mismatch_details=True,
                                                                   amz_seller_ept=seller and seller.id or False)
                    continue
                existing_order = self.search([('amz_order_reference', '=', amazon_order_ref),
                                              ('amz_instance_id', '=', instance.id),
                ...
                if existing_order:
                    message = 'Amazon order Ref %s is already available in the system.' % (existing_order.name)
                    common_log_line_obj.create_common_log_line_ept(message=message, model_name=SALE_ORDER,
                                                                   fulfillment_by='FBA', order_ref=amazon_order_ref,
                                                                   module='amazon_ept', operation_type='import',
                                                                   res_id=self.id,
    ...
    def check_amazon_order_vals_ept(seller, order, marketplace_instance_dict):
        ...
        amazon_order_ref = order.get('AmazonOrderId', False)
        if not amazon_order_ref:
            message = ('Amazon order reference is not found in the response.\n'
                       'Action Items:\n'
                       '-Ensure the correct Amazon Order Ref is being used.\n'
                       "-If this continues, reprocess the order after confirming it's present on Amazon.")
        ...
        if not instance:
            ...
            marketplace_instance_dict.update({marketplace_id: instance})
            if not instance:
                message = ('Amazon order was not imported as the Amazon instance is not found in the system.\n'
                           'Action Items:\n'
                           '-Verify the active marketplace under: Amazon → Configuration → Marketplaces.\n'
                           '-If the required marketplace is missing, you can load it from the seller account.')
    ...
    def prepare_amazon_sale_order_vals(self, instance, partner_dict, order):
        ...
        earliest_ship_date = (
            parser.parse(order.get('EarliestShipDate')).astimezone(utc).strftime(DATE_YMDHMS)
            if order.get('EarliestShipDate') else datetime.now(utc).strftime(DATE_YMDHMS)
        )
        earliest_delivery_date = (
            parser.parse(order.get('EarliestDeliveryDate')).astimezone(utc).strftime(DATE_YMDHMS)
            if order.get('EarliestDeliveryDate') else datetime.now(utc).strftime(DATE_YMDHMS)
        )
    ...
    def create_or_find_amazon_product(self, order_details, product_details, instance, amazon_order_ref,
        ...
        if not seller_sku:
            skip_order = True
            message = ('Amazon order Ref could not be processed as the Seller SKU is missing in the order response.\n'
                       'Action Items:\n'
                       '-Confirm that the product on Amazon has a valid Seller SKU.\n'
                       '-If missing, update the listing on Amazon and reimport the order.')
            common_log_line_obj.create_common_log_line_ept(message=message, model_name=SALE_ORDER, mismatch_details=True,
    ...
    def amz_search_or_create_odoo_product(self, instance, amazon_order_ref, fulfillment_by,
        ...
        if not instance.seller_id.create_new_product:
            ...
            message = ('System could not find a matching Odoo product for Seller SKU %s.\n'
                       'Action Items:\n'
                       '-Check if the SKU in the order exactly matches the product’s internal reference in Odoo.\n'
                       '-Ensure system configuration allows auto-creation of products.\n'
                       '-Alternatively, manually create the product in Odoo with Internal Reference %s and reimport '
                       'the order.') % (seller_sku, seller_sku)
            ...
            erp_prod_vals = {'name': order_details.get('Title', ''),
                             'default_code': seller_sku, 'type': 'consu',
                             'purchase_ok': True, 'sale_ok': True, 'is_storable': True}
            odoo_product = product_obj.create(erp_prod_vals)
            message = 'Amazon product was successfully created with Seller Reference %s for instance %s.' % (seller_sku, instance.name)
    ...
    def prepare_updated_ordervals(instance, order_ref, order):
        ...
        ordervals = {
            ...
            'auto_workflow_process_id': instance.fbm_marketplace_workflow_id.id if instance.is_marketplace_workflow
            else instance.seller_id.fbm_auto_workflow_id.id,
    ...
    def amz_create_sales_order(self, queue_order):
        ...
        for line in queue_lines:
            ...
            if not order.get('PurchaseDate', ''):
                message = ('Purchase date was not received in the response for Amazon order ref: %s.\n'
                           'Action Items:\n'
                           '-The response from Amazon might be incomplete.\n'
                           '-Verify data manually in the order file and queue.\n'
                           '-You can reimport the unshipped order again.') % amazon_order_ref
                log_line_ept.create_common_log_line_ept(message=message,
                                                        model_name='shipped.order.data.queue.ept',
                                                        fulfillment_by='FBM', module='amazon_ept',
                                                        operation_type='import', res_id=queue_order.id,
            ...
            if not order.get('BuyerInfo', {}).get('BuyerEmail', ''):
                message = ("Buyer's details were not received in the response for Amazon order ref: %s.\n"
                           "Action Items:\n"
                           "-The response from Amazon might be incomplete.\n"
                           "-Verify data manually in the order file and queue.\n"
                           "-You can reimport the unshipped order again.") % amazon_order_ref
            ...
            if fulfillment_channel and fulfillment_channel == 'AFN' and \
                ...
                message = 'System skipped the Amazon order ref: %s as the fulfillment channel is AFN' % amazon_order_ref
                log_line_ept.create_common_log_line_ept(message=message,
                                                        model_name='shipped.order.data.queue.ept',
                                                        fulfillment_by='FBM', module='amazon_ept',
                                                        operation_type='import', res_id=queue_order.id,
    ...
    def process_existing_order_cancel_request(list_of_shipped_order_lines, existing_order, line):
        ...
        for order_line_list in list_of_shipped_order_lines:
            ...
            cancel_requests = [x.get('BuyerRequestedCancel') for x in order_lines if
                               ast.literal_eval(x.get('BuyerRequestedCancel', {}).
                                    get('IsBuyerRequestedCancel', 'false').capitalize())]
            if cancel_requests:
                existing_order.buyer_requested_cancellation = ast.literal_eval(cancel_requests[0].
                                                                   get('IsBuyerRequestedCancel', 'false').capitalize())
    ...
    def get_fbm_next_token_orders(self, seller, next_token):
        ...
        if response.get('error', False):
            ...
            try:
                ...
                raise UserError(f"An error Occurred during the importing Fbm Orders : {str(response.get('error', {}))}")
        ...
        for order_line_wrapper_obj in list_of_shipped_order_lines:
            ...
            if sales_order:
                ...
                if order_status == 'Shipped':
                    workflow = instance.fbm_marketplace_workflow_id if instance.is_marketplace_workflow else (
                        sales_order.amz_seller_id.fbm_auto_workflow_id)
                    workflow.shipped_order_workflow_ept(sales_order)
                elif order_status in ['Unshipped', 'PartiallyShipped']:
                    sales_order.process_orders_and_invoices_ept()
                else:
                    message = ('System did not execute auto-invoice workflow for Amazon order ref: %s with order '
                               'status: %s') % (sales_order.amz_order_reference, order_status)
    ...
    def process_shipped_or_missing_unshipped_order_ept(self, instance, order, order_lines, line_state, queue_order):
        ...
        cancel_requests = [x.get('BuyerRequestedCancel') for x in order_lines if ast.literal_eval(x.get(
        ...
        for order_line in order_lines:
            ...
            if float_is_zero(float(order_line.get('QuantityOrdered', 0.0)), precision_digits=2):
                message = 'Skipped Order line because of 0 Quantity Ordered.'

                common_log_line_ept.create_common_log_line_ept(
                    message=message, model_name='shipped.order.data.queue.ept', fulfillment_by='FBM',
                    module='amazon_ept', operation_type='import', res_id=queue_order.id, mismatch_details=False,
            ...
            if not sales_order or sales_order.amz_order_reference != amazon_order_ref:
                ...
                if partner:
                    ...
                    if cancel_requests:
                        ordervals.update({
                            'buyer_requested_cancellation': ast.literal_eval(cancel_requests[0].get(
    ...
    def create_amazon_shipped_or_unshipped_order_vals(self, instance, partner, order):
        ...
        ordervals.update({'picking_policy': instance.fbm_marketplace_workflow_id.picking_policy if
        instance.is_marketplace_workflow else instance.seller_id.fbm_auto_workflow_id.picking_policy})
    ...
    def auto_process_shipped_order_queue_line(self, data_queue):
        ...
        if data_queue.shipped_order_data_queue_lines:
            cron_id = self.env.ref('amazon_ept.ir_cron_child_to_process_shipped_order_queue_line', raise_if_not_found=False)
    ...
    def prepare_sale_order_update_values(instance, order):
        ...
        is_prime_order = True if str(order.get('IsPrime', '')).lower() in ['true', 't'] else False
        return {
            'auto_workflow_process_id': instance.fba_marketplace_workflow_id.id if instance.is_marketplace_workflow
            else instance.seller_id.fba_auto_workflow_id.id or False,
            'amz_instance_id': instance and instance.id or False,
            'amz_fulfillment_by': 'FBA',
    ...
    def create_amazon_sales_order_vals(self, partner_dict, order, instance):
        ...
        order_vals.update({'picking_policy': instance.fba_marketplace_workflow_id.picking_policy if
        instance.is_marketplace_workflow else instance.seller_id.fba_auto_workflow_id.picking_policy})
    ...
    def amz_create_common_log(self, message, instance, order_ref, fulfillment_by, res_id, model_name):
        ...
        common_log_obj.create_common_log_line_ept(
            ...
            operation_type='import', res_id=res_id,
            amz_instance_ept=instance.id, mismatch_details=True,
    ...
    def create_amazon_shipping_report_sale_order(self, row, partner, report_id):
        ...
        # set picking policy as FBA Auto workflow picking policy
        order_vals.update({'picking_policy': instance.fba_marketplace_workflow_id.picking_policy if
        instance.is_marketplace_workflow else instance.seller_id.fba_auto_workflow_id.picking_policy or False})
        carrier_id = False
        if row.get('carrier', ''):
            ...
            if not carrier_id:
                ...
                self.amz_create_common_log(message, instance, row.get('amazon-order-id', False), 'FBA',
    ...
    def amz_mismatch_log_for_update_order_status(self, message, order):
        """
        Define this method for creating the log lines for any mismatch found in the order status process.
        :param: message: str
        :param: order : sale.order()
        :return:
        """
        if self._context.get('auto_process', False):
            common_log_obj = self.env[COMMON_LOG_LINES_EPT]
            common_log_obj.create_common_log_line_ept(message=message, model_name='sale.order', module='amazon_ept',
                                                      fulfillment_by='FBM', order_ref=order.amz_order_reference,
                                                      operation_type='export', res_id=order.id,
                                                      amz_instance_ept=order.amz_instance_id.id,
                                                      amz_seller_ept=order.amz_instance_id.seller_id.id if
                                                      order.amz_instance_id.seller_id else False)
        else:
    ...
    def get_amz_ship_info_and_parcel_values(self, picking, amazon_order, fulfillment_date_concat):
        ...
        if not amazon_order.warehouse_id:
            message = ('System tried to find the warehouse while preparing data to update order status for Amazon order '
                       'ref: %s but not found.\n'
                       'Action Items:\n'
                       '-Set the proper warehouse in the Amazon order ref: %s\n'
                       '-Try performing the update order status again') % (amazon_order.amz_order_reference, amazon_order.amz_order_reference)
            self.amz_mismatch_log_for_update_order_status(message, amazon_order)
        partner = amazon_order.warehouse_id.partner_id
        if not partner:
            message = ('System tried to find the address of the warehouse: %s, but it was not found.\n'
                       'Action Items:\n'
                       '-Open the warehouse: Inventory -> Configuration -> Settings -> Warehouse.\n'
                       '-Set the proper address in the warehouse.\n'
                       '-Try performing the update order status again' % amazon_order.warehouse_id.name)
            self.amz_mismatch_log_for_update_order_status(message, amazon_order)
        elif partner.name and re.search(r'[^\w\s]', partner.name):
            message = ('System tried to prepare address data to update order status for the warehouse: %s '
                       'but found a special character in the address: %s.\n'
                       'Action Items:\n'
                       '-Open the warehouse: Inventory -> Configuration -> Settings -> Warehouse.\n'
                       '-Open the warehouse address and remove the special character from the address.\n'
                       '-Try performing the update order status again') % (amazon_order.warehouse_id.name, partner.name)
            self.amz_mismatch_log_for_update_order_status(message, amazon_order)
        elif not all([partner.street, partner.city, partner.state_id, partner.zip, partner.country_id]):
            message = ("System tried to prepare address data to update order status for the warehouse: %s but "
                       "did not find address details for Street, City, State, Zip, or Country for address: %s.\n"
                       "Action Items:\n"
                       "-Open the warehouse: Inventory -> Configuration -> Settings -> Warehouse.\n"
                       "-Set complete address details.\n"
                       "-Try performing the update order status again.") % (amazon_order.warehouse_id.name, partner.name)
            self.amz_mismatch_log_for_update_order_status(message, amazon_order)
        else:
    ...
    def get_amz_message_information_ept(self, amazon_orders):
        ...
        for amazon_order in amazon_orders:
            for picking in amazon_order.picking_ids:
                ...
                multi_tracking = True if picking.mapped("move_line_ids.result_package_id").filtered(
                    lambda l: l.tracking_no) else False
    ...
    def amz_prepare_phantom_product_dict_ept(self, picking_ids):
        ...
        moves = move_obj
        drop_ship_mod = self.env['product.product'].search_installed_module_ept('stock_dropshipping')
        domain = [('state', 'not in', ['draft', 'cancel']), ('updated_in_amazon', '=', False)]
        if drop_ship_mod:
            drop_ship_picks = picking_ids.filtered(lambda p: p.is_dropship)
            if drop_ship_picks:
                moves += move_obj.search([('picking_id', 'in', drop_ship_picks.ids),
                                          ('picking_type_id.code', '=', 'incoming')] + domain)
                picking_ids = picking_ids - drop_ship_picks if drop_ship_picks else picking_ids
        if picking_ids:
            moves += move_obj.search([('picking_id', 'in', picking_ids.ids),
                                      ('picking_type_id.code', '!=', 'incoming')] + domain)
    ...
    def get_qty_for_phantom_type_products(self, order, picking, message_id, fulfillment_date_concat):
        ...
        move_obj = self.env['stock.move']
        update_move_ids = []
        picking_ids = order.picking_ids.filtered(lambda l: l.location_dest_id.usage == 'customer')
    ...
    def onchange_fba_warehouse(self):
        ...
        if self.warehouse_id.is_fba_warehouse and not self.auto_workflow_process_id:
            self.auto_workflow_process_id = self.amz_instance_id.fba_marketplace_workflow_id.id if (
                self.amz_instance_id.is_marketplace_workflow) else self.warehouse_id.seller_id.fba_auto_workflow_id.id
    ...
    def create_outbound_order(self):
        ...
        for order in self:
            ...
            if 'service' in prod_types and len(prod_types) == 1:
                ...
                return False
            if not order.exported_in_amazon:
    ...
    def create_fulfillment(self):
        ...
        if orders:
            ...
            for order in filtered_orders:
                ...
                response = iap_tools.iap_jsonrpc(DEFAULT_ENDPOINT, params=kwargs, timeout=1000)
                self.with_context({'amz_outbound_order_ept': order.id}).process_create_outbound_order_response(
                    response, is_auto_process)
    ...
    def process_create_outbound_order_response(self, response, is_auto_process):
        ...
        """
        common_log_book_obj = self.env['common.log.book.ept']
        amz_order_id = self.browse(self._context.get('amz_outbound_order_ept', False))
        if response.get('error', False):
            if amz_order_id and amz_order_id.warehouse_id.seller_id and amz_order_id.warehouse_id.seller_id.is_create_schedule_activity_for_outbound:
                common_log_book_obj.amz_create_schedule_activity_for_failed_outbound_order(
                    amz_order_id.warehouse_id.seller_id, 'sale.order', amz_order_id.id, response.get('error', False),
                    amz_order_id.client_order_ref)
            else:
                if is_auto_process:
