...
class AmazonConfigSettings(models.TransientModel):
    ...
    def switch_amazon_fulfillment_by(self):
        ...
        ctx = result.get('context', {}) and ast.literal_eval(result.get('context', {}))
    ...
    is_create_schedule_activity_for_outbound = fields.Boolean("Create Schedule Activity for failed order? ", default=False,
                                                     help="If checked, Then Schedule Activity create for failed "
                                                          "outbound order.")
    ...
    def onchange_amz_seller_id(self):
        ...
        if self.amz_seller_id:
            ...
            vals['value']['is_create_schedule_activity_for_outbound'] = seller.is_create_schedule_activity_for_outbound or False
            vals['value']['amz_outbound_activity_type_id'] = seller.amz_outbound_activity_type_id or False
            vals['value']['amz_outbound_activity_date_deadline'] = seller.amz_outbound_activity_date_deadline or False
    ...
    def execute(self):
        ...
        # No longer want to set the analytic account for seller and instances.
        ...
        if self.amz_seller_id:
            if not self.allow_auto_create_outbound_orders:
                self.amz_create_outbound_on_confirmation = False
                self.is_create_schedule_activity_for_outbound = False
            ...
            vals['is_create_schedule_activity_for_outbound'] = self.is_create_schedule_activity_for_outbound \
                if self.allow_auto_create_outbound_orders else False
            vals['amz_outbound_activity_type_id'] = self.amz_outbound_activity_type_id
            vals['amz_outbound_activity_date_deadline'] = self.amz_outbound_activity_date_deadline
            vals[
    ...
    def get_iap_pack_url_based_subscription_token_ept(self):
        ...
        if not sellers:
            raise UserError('Please register your seller.')
