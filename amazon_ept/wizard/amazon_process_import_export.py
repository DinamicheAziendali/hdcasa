...
class AmazonProcessImportExport(models.TransientModel):
    ...
    fba_operations = fields.Selection([
        ...
        ('amz_replacement_orders_report', 'Import Replacement Orders Report'),
        ('amz_import_awd_inbound_shipment', 'Import AWD Inbound Shipment'),
    ], 'Operations')
    ...
    import_by = fields.Selection(
        [('with_shipment_id', 'Shipment Id'),
         ('with_date_range', 'Date Range')])
    ...
    def onchange_fba_operations(self):
        ...
        if self.fba_operations == "amz_replacement_orders_report":
            self.report_start_date = datetime.now() - timedelta(
                self.seller_id.replacement_order_report_days)
            self.report_end_date = datetime.now()
    ...
    def amz_list_settlement_report(self):
        ...
        vals = {'report_type': ReportType.GET_V2_SETTLEMENT_REPORT_DATA_FLAT_FILE_V2.value,
    ...
    def amz_replacement_orders_report(self):
        """
        This method check scheduler running or not for import FBA Replacement Orders Report if running
        than raise warning otherwise import FBA Replacement Orders Report from Amazon to odoo.
        :return: ir.actions.act_window() action
        """
        replacement_orders_report_obj = self.env['replacement.order.report.request.history']
        # here add scheduler reference as per define scheduler id
        self.with_context({'raise_warning': True}).check_running_schedulers(
            'ir_cron_create_fba_replacement_orders_report_seller_')
        if not self.report_start_date or not self.report_end_date:
            raise UserError(_(DATE_VALIDATION_MESSAGE))
        replacement_report_records = replacement_orders_report_obj.amz_create_replacement_order_report(
            self.seller_id, self.report_start_date, self.report_end_date, self.instance_ids)
        if not replacement_report_records:
            return True
        return self.amz_return_tree_form_action_ept(
            'Replacement Order Report Request History',
            'replacement.order.report.request.history', replacement_report_records)
    ...
    def amz_import_inbound_shipment(self):
        ...
        if not inbound_shipment_list:
            return
        return self.amz_return_tree_form_action_ept(
            'Inbound Shipments', 'inbound.shipment.new.ept', inbound_shipment_list)

    def amz_import_awd_inbound_shipment(self):
        """
        This method check scheduler running or not for import AWD Inbound Shipment Report if running than raise
        warning otherwise import AWD Inbound Shipment Report from Amazon to odoo.
        """
        awd_inbound_shipment_list = []
        awd_inbound_shipment_obj = self.env['awd.inbound.shipment.ept']
        if self.import_by == 'with_shipment_id':
            awd_inbound_shipment_list = awd_inbound_shipment_obj.get_awd_inbound_import_shipment(self.instance_id,
            ...
            awd_inbound_shipment_result = awd_inbound_shipment_obj.get_awd_inbound_import_shipment_data_wise(
                self.instance_id, self.from_warehouse_id, self.report_start_date, self.report_end_date,
                self.ship_to_address)
            if awd_inbound_shipment_result:
                ...
                for shipment_result in awd_shipments:
                    ...
                    awd_inbound_shipment_list = awd_inbound_shipment_obj.get_awd_inbound_import_shipment(
                        self.instance_id,
                        self.from_warehouse_id,
                        shipment_id,
                        self.ship_to_address, self.to_warehouse_id)
        if not awd_inbound_shipment_list:
            return
        return self.amz_return_tree_form_action_ept(
            'AWD Shipments', 'awd.inbound.shipment.ept', awd_inbound_shipment_list)
    ...
    def get_reports(self, report_values):
        ...
        :return: This method will redirecting us to settlement report list view.
        """
    ...
    def amz_manually_create_settlement_report(self):
        ...
        settlement_exist = settlement_report_obj.search(
            [('amz_settlement_ref', '=', settlement_report_record.amz_settlement_ref),
             ('id', '!=', settlement_report_record.id)], order='id')
        if settlement_exist:
            settlement_report_record.write({'already_processed_report_id': settlement_exist[0].id, 'state': 'duplicate'})
        attachment_id = self.create_attachment_for_settlement_report(settlement_report_record)
        settlement_report_record.message_post(body=_("Settlement Record has been created manually."),
    ...
    def prepare_settlement_report_vals(self):
        ...
        for row in settlement_reader:
            ...
            settlement_ref = row.get('settlement-id', False)
    ...
    def amz_mapped_imports_products_ept(self, line, instance_dict, map_product_count):
        ...
        if fulfillment not in ['FBA', 'FBM']:
            message = ("Fulfillment type is not set properly. Received fulfillment value: %s is invalid.\n"
                       "Action Items:\n"
                       "-Fulfillment should be either FBA or FBM.\n"
                       "-Update the correct fulfillment type in the file.\n"
                       "-Reprocess the file from: Operations → Map Products.") % fulfillment
            common_log_line_obj.create_common_log_line_ept(
                message=message, model_name='product.product', default_code=seller_sku,
        ...
        if not instance:
            message = ("Amazon marketplace name is not set correctly. Received marketplace name: %s is invalid.\n"
                       "Action Items:\n"
                       "-Marketplace name must match an enabled marketplace in the system.\n"
                       "-You can check valid marketplaces at: Configuration → Marketplaces.\n"
                       "-Correct the marketplace name in the file.\n"
                       "-Reprocess the file from: Operations → Map Products.") % amazon_marketplace
            common_log_line_obj.create_common_log_line_ept(
        ...
        if instance and fulfillment and seller_sku:
            if not odoo_default_code:
                message = ("Internal reference not found while processing the file for seller SKU: %s.\n"
                           "Action Items:\n"
                           "-Set the correct Internal Reference (SKU) for the product mentioned in the file.\n"
                           "-Reprocess the file from: Operations → Map Products.") % seller_sku
                common_log_line_obj.create_common_log_line_ept(
                    message=message, model_name='product.product', default_code=seller_sku, fulfillment_by=fulfillment,
                    mismatch_details=True, module='amazon_ept', operation_type='import',
                    amz_seller_ept=self.seller_id and self.seller_id.id or False)
            ...
            if amazon_product_id and amazon_product_id.product_id.id != product_id.id:
                ...
                if product_id:
                    ...
                    message = ("Internal reference %s is not found in the system while mapping products.\n"
                               "Action Items:\n"
                               "-Create a new product in Odoo with the internal reference %s OR,\n"
                               "-Assign the internal reference %s to an existing Odoo product.\n"
                               "-After updating the product, reprocess the file from: Operations → Map Products."
                               ) % (odoo_default_code, odoo_default_code, odoo_default_code)
                    mismatch = True
                common_log_line_obj.create_common_log_line_ept(
                    message=message, model_name='product.product', default_code=seller_sku, fulfillment_by=fulfillment,
            ...
            map_product_count += 1
            message = """ Amazon product has been created with Seller Reference %s for instance %s, and linked to Odoo product %s. """ % (seller_sku, instance.name, product_id.name)
            common_log_line_obj.create_common_log_line_ept(
    ...
    def get_odoo_product_csv_data_ept(self, line_vals, fullfillment_by):
        ...
        if not product_id:
            ...
            common_log_line_obj.create_common_log_line_ept(
                ...
                product_title=line_vals.get('Title', False), amz_seller_ept=self.seller_id and self.seller_id.id or False,
                odoo_internal_reference=odoo_default_code
            )
        return product_id
    ...
    def amz_create_inbound_shipment_plan(self):
        ...
        form_id = self.env.ref('amazon_ept.inbound_shipment_plan_new_form_view')
        warehouse_id = instance.warehouse_id
        vals = {'instance_id': instance.id,
                'warehouse_id': warehouse_id.id,
                'source_address_id': warehouse_id.partner_id and warehouse_id.partner_id.id,
                'company_id': instance.company_id and instance.company_id.id,
                'is_box_information_known': instance.is_inbound_box_information_known if instance.is_inbound_box_information_known else False
                }
        shipment_plan_id = inbound_shipment_plan_obj.create(vals)
        return {
            'type': IT_ACTIONS_ACT_WINDOW,
            'name': 'Inbound Shipment Plan',
            'res_model': 'inbound.shipment.plan.new.ept',
