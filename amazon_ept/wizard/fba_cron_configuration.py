...
class FbaCronConfiguration(models.TransientModel):
    ...
    # Request FBA Replacement Orders Report
    auto_create_fba_repl_orders_report = fields.Boolean(
        string="Auto Request FBA Replacement Orders Report ?")
    fba_repl_orders_report_next_execution = fields.Datetime(
        'Auto Create Replacement Orders Report Next Execution',
        help='Replacement Orders Report Next Execution Time')
    fba_repl_orders_report_interval_number = fields.Integer(
        'Auto Create Replacement Orders Report Interval Number', help="Repeat every x.")
    fba_repl_orders_report_interval_type = fields.Selection([('hours', 'Hours'), ('days', 'Days')],
                                                            'Auto Create Replacement Orders Report Interval Unit')
    fba_repl_orders_report_user_id = fields.Many2one(RES_USERS,
                                                     string="Import FBA Replacement Orders Report User")

    # AWD shipment cron
    amz_auto_import_awd_shipment = fields.Boolean(string='Auto Import FBA AWD Shipment?')
    amz_awd_shipment_status_import_next_execution = fields.Datetime( \
        'Auto Import FBA AWD Shipment Next Execution', help='Next execution time')
    amz_awd_shipment_status_import_interval_number = fields.Integer( \
        'Auto Import FBA AWD Shipment Interval Number', help="Repeat every x.")
    amz_awd_shipment_status_import_interval_type = fields.Selection([('minutes', 'Minutes'),
                                                                     ('hours', 'Hours'),
                                                                     ('days', 'Days'),
                                                                     ('weeks', 'Weeks'),
                                                                     ('months', 'Months')],
                                                                    'Auto Import FBA AWD Shipment Status Unit')
    ...
    def update_amz_awd_shipment_import_cron_field(self, amz_seller):
        """
        This methdo is used to Update the AWD shipment cron values based on configuration.
        :param amz_seller:
        :return:
        """
        if amz_seller:
            amz_awd_shipment_cron = self.env.ref(IMPORT_AWD_SHIPMENT % amz_seller.id, raise_if_not_found=False)
            if amz_awd_shipment_cron:
                self.amz_auto_import_awd_shipment = amz_awd_shipment_cron.active or False
                self.amz_awd_shipment_status_import_next_execution = amz_awd_shipment_cron.nextcall or False
                self.amz_awd_shipment_status_import_interval_number = amz_awd_shipment_cron.interval_number or False
                self.amz_awd_shipment_status_import_interval_type = amz_awd_shipment_cron.interval_type or False
                self.amz_awd_shipment_status_import_user_id = amz_awd_shipment_cron.user_id.id or False

    def update_amz_fba_repl_orders_auto_import_cron_field(self, amz_seller):
        """
        This method is used to update amazon import FBA replacement orders cron values based on configurations.
        :param amz_seller : seller record.
        """
        if amz_seller:
            amz_check_replacement_report_cron_exist = self.env.ref(
                'amazon_ept.ir_cron_create_fba_replacement_orders_report_seller_%d' % (amz_seller.id),
                raise_if_not_found=False)
            if amz_check_replacement_report_cron_exist:
                self.auto_create_fba_repl_orders_report = (
                        amz_check_replacement_report_cron_exist.active or False)
                self.fba_repl_orders_report_interval_number = (
                        amz_check_replacement_report_cron_exist.interval_number or False)
                self.fba_repl_orders_report_interval_type = (
                        amz_check_replacement_report_cron_exist.interval_type or False)
                self.fba_repl_orders_report_next_execution = (
                        amz_check_replacement_report_cron_exist.nextcall or False)
                self.fba_repl_orders_report_user_id = (
                        amz_check_replacement_report_cron_exist.user_id.id or False)

    def setup_replacement_orders_report_create_cron(self, seller):
        """
        This method will active the cron to import replacement orders report.
        param amazon_seller : seller record.
        """
        if self.auto_create_fba_repl_orders_report:
            cron_exist = self.env.ref(
                'amazon_ept.ir_cron_create_fba_replacement_orders_report_seller_%d' % (seller.id),
                raise_if_not_found=False)
            vals = {
                'active': True,
                'interval_number': self.fba_repl_orders_report_interval_number,
                'interval_type': self.fba_repl_orders_report_interval_type,
                'nextcall': self.fba_repl_orders_report_next_execution,
                'code': "model.auto_import_replacement_orders_report({'seller_id':%d})" % (seller.id),
                'amazon_seller_cron_id': seller.id,
                'user_id': self.fba_repl_orders_report_user_id and self.fba_repl_orders_report_user_id.id,
            }
            if cron_exist:
                cron_exist.write(vals)
            else:
                repl_report_cron = self.env.ref('amazon_ept.ir_cron_create_fba_replacement_orders_report',
                                                raise_if_not_found=False)
                if not repl_report_cron:
                    raise UserError(_(
                        'Core settings of Amazon are deleted, please upgrade Amazon module to back this settings.'))
                name = 'FBA-' + seller.name + ' : Create Amazon Replacement Orders Report'
                vals.update({'name': name})
                new_cron = repl_report_cron.copy(default=vals)
                model_name = 'ir_cron_create_fba_replacement_orders_report_seller_%d' % (seller.id)
        ...
        self.setup_replacement_orders_report_process_cron(seller)
        return True

    def setup_replacement_orders_report_process_cron(self, seller):
        """
        This method will active the cron to process replacement orders report.
        param amazon_seller : seller record.
        """
        if self.auto_create_fba_repl_orders_report:
            cron_exist = self.env.ref(
                'amazon_ept.ir_cron_process_fba_repl_orders_report_seller_%d' % (seller.id), raise_if_not_found=False)
            process_next_execution = self.fba_repl_orders_report_next_execution + relativedelta(minutes=10)
            vals = {
                'active': True,
                'interval_number': self.fba_repl_orders_report_interval_number,
                'interval_type': self.fba_repl_orders_report_interval_type,
                'nextcall': process_next_execution,
                'code': "model.auto_process_replacement_orders_report({'seller_id':%d})" % (seller.id),
                'amazon_seller_cron_id': seller.id,
                'user_id': self.fba_repl_orders_report_user_id and self.fba_repl_orders_report_user_id.id,
            }
            if cron_exist:
                ...
                repl_report_cron = self.env.ref('amazon_ept.ir_cron_process_fba_replacement_orders_report',
                                                raise_if_not_found=False)
                if not repl_report_cron:
                    raise UserError(_(
                        'Core settings of Amazon are deleted, please upgrade Amazon module to back this settings.'))

                name = 'FBA-' + seller.name + ' : Process Replacement Orders Report'
                vals.update({'name': name})
                new_cron = repl_report_cron.copy(default=vals)
                model_name = 'ir_cron_process_fba_repl_orders_report_seller_%d' % (seller.id)
                self.create_amazon_fba_scheduler(new_cron.id, model_name)
        else:
            cron_exist = self.env.ref(
                'amazon_ept.ir_cron_process_fba_repl_orders_report_seller_%d' % (seller.id), raise_if_not_found=False)
    ...
    def update_amz_new_inbound_shipment_status(self, amz_seller):
        """
        This method is used to update the inbound shipment status cron values based on
        configurations.
        :param amz_seller : seller record.
        """
        if amz_seller:
            amz_inbound_status_cron = self.env.ref(
                'amazon_ept.ir_cron_auto_check_status_new_inbound_shipment_%d' % amz_seller.id,
                raise_if_not_found=False)
            if amz_inbound_status_cron:
                self.amz_auto_import_inbound_shipment_status = amz_inbound_status_cron.active or False
                self.amz_shipment_status_import_next_execution = \
    ...
    def setup_awd_inbound_shipment_status_cron(self, seller):
        """
        This method will active the cron to import FBA shipment status.
        param amazon_seller : seller record.
        """
        self.auto_process_awd_inbound_shipment_report(seller)
        if self.amz_auto_import_awd_shipment:
            cron_exist = self.env.ref(IMPORT_AWD_SHIPMENT % (seller.id), raise_if_not_found=False)
            vals = {
                'active': True,
                'interval_number': self.amz_awd_shipment_status_import_interval_number,
                'interval_type': self.amz_awd_shipment_status_import_interval_type,
                'nextcall': self.amz_awd_shipment_status_import_next_execution,
                'code': "model.auto_import_awd_shipment({'seller_id':%d})" % (seller.id),
                'amazon_seller_cron_id': seller.id,
            }
            ...
            if cron_exist:
                ...
                import_awd_shipment_cron = self.env.ref('amazon_ept.ir_cron_auto_import_awd_inbound_shipment_report',
                                                        raise_if_not_found=False)
                if not import_awd_shipment_cron:
                    raise UserError(
                        _('Core settings of Amazon AWD Inbound Shipment are deleted, please upgrade Amazon module to back this settings.'))

                name = 'FBA-' + seller.name + ' : Import Amazon AWD Shipment Report'
                vals.update({'name': name})
                new_cron = import_awd_shipment_cron.copy(default=vals)
                model_name = 'ir_cron_auto_import_awd_inbound_shipment_report_seller_%d' % (seller.id)
                self.create_amazon_fba_scheduler(new_cron.id, model_name)
        else:
            cron_exist = self.env.ref(IMPORT_AWD_SHIPMENT % (seller.id), raise_if_not_found=False)
            if cron_exist:
    ...
    def auto_process_awd_inbound_shipment_report(self, seller):
        """
        This method will activate the cron to process AWD Inbound shipment report.
        param amazon_seller : seller record.
        """
        if self.amz_auto_import_awd_shipment and self.amazon_selling not in ['FBM']:
            cron_exist = self.env.ref(
                'amazon_ept.ir_cron_auto_process_awd_inbound_shipment_report_seller_%d' % (seller.id),
                raise_if_not_found=False)
            process_next_execution = self.amz_awd_shipment_status_import_next_execution + relativedelta(
                minutes=10)
            vals = {
                'active': True,
                'interval_number': self.amz_awd_shipment_status_import_interval_number,
                'interval_type': self.amz_awd_shipment_status_import_interval_type,
                'nextcall': process_next_execution,
                'user_id': self.amz_awd_shipment_status_import_user_id.id,
                'code': "model.auto_process_awd_shipment_report({'seller_id':%d, ""'is_auto_process': True})" % (
                    seller.id),
                'amazon_seller_cron_id': seller.id
            }
            if cron_exist:
                ...
                import_order_cron = self.env.ref('amazon_ept.ir_cron_auto_process_awd_inbound_shipment_report',
                                                 raise_if_not_found=False)
                if not import_order_cron:
                    raise UserError(
                        _('Core settings of Amazon process shipment report are deleted, please upgrade Amazon module to back this settings.'))

                name = 'FBA-' + seller.name + ' : Amazon AWD shipment check status'
                vals.update({'name': name})
                new_cron = import_order_cron.copy(default=vals)
                model_name = 'ir_cron_auto_process_awd_inbound_shipment_report_seller_%d' % (seller.id)
                self.create_amazon_fba_scheduler(new_cron.id, model_name)
        else:
            cron_exist = self.env.ref(
                'amazon_ept.ir_cron_auto_process_awd_inbound_shipment_report_seller_%d' % (seller.id),
    ...
    def setup_inbound_shipment_status_cron(self, seller):
        ...
        if self.amz_auto_import_inbound_shipment_status:
            ...
            if cron_exist:
                ...
                self.amz_setup_all_auto_check_status_new_inbound_shipment_cron(seller, vals)
                self.amz_setup_new_inbound_shipment_check_status_cron(seller, vals)
            else:
            ...
            new_shipment_check_status_cron_exist = self.env.ref(
                'amazon_ept.ir_cron_auto_check_status_new_inbound_shipment_%d' % (seller.id), raise_if_not_found=False)
            all_new_check_shipment_status = self.env.ref(
                'amazon_ept.ir_cron_auto_check_status_all_new_inbound_shipment_%d' % (seller.id),
                raise_if_not_found=False)
            if cron_exist:
                cron_exist.write({'active': False})
            if all_check_status_cron_exist:
                all_check_status_cron_exist.write({'active': False})
            if new_shipment_check_status_cron_exist:
                new_shipment_check_status_cron_exist.write({'active': False})
            if all_new_check_shipment_status:
                all_new_check_shipment_status.write({'active': False})
        return True
    ...
    def amz_setup_all_auto_check_status_new_inbound_shipment_cron(self, seller, cron_values):
        """
        Define this method for active a scheduler for check all new inbound shipment status.
        :param: seller: amazon.seller.ept()
        :param: cron_values: dict {}
        :return: True
        """
        cron_values.update({
            'code': "model.auto_all_check_status_new_inbound_shipment({'seller_id':%d})" % seller.id})
        cron_exist = self.env.ref(
            'amazon_ept.ir_cron_auto_check_status_all_new_inbound_shipment_%d' % seller.id, raise_if_not_found=False)
        if cron_exist:
            cron_exist.write(cron_values)
        else:
            auto_inbound_cron = self.env.ref(
                'amazon_ept.ir_cron_all_auto_check_status_new_inbound_shipment', raise_if_not_found=False)
            if not auto_inbound_cron:
                raise UserError(_(
                    'Core settings of Amazon import all shipment status are deleted, please upgrade '
                    'Amazon module to back this settings.'))
            name = 'FBA-' + seller.name + ' : Amazon All New inbound shipment Check status'
            cron_values.update({'name': name})
            new_cron = auto_inbound_cron.copy(default=cron_values)
            model_name = 'ir_cron_auto_check_status_all_new_inbound_shipment_%d' % seller.id
            self.create_amazon_fba_scheduler(new_cron.id, model_name)
        return True

    def amz_setup_new_inbound_shipment_check_status_cron(self, seller, cron_values):
        """
        Define this method for active a scheduler for check all inbound shipment status.
        :param: seller: amazon.seller.ept()
        :param: cron_values: dict {}
        ...
        """
        cron_values.update({
            'code': "model.auto_import_new_shipment_status_ept({'seller_id':%d})" % (seller.id)})
        cron_exist = self.env.ref(
            'amazon_ept.ir_cron_auto_check_status_new_inbound_shipment_%d' % (seller.id), raise_if_not_found=False)
        if cron_exist:
            cron_values.update({'name': cron_exist.name})
            cron_exist.write(cron_values)
        else:
            auto_inbound_cron = self.env.ref(
                'amazon_ept.ir_cron_auto_new_inbound_shipment_check_status', raise_if_not_found=False)
            if not auto_inbound_cron:
                raise UserError(_(
                    'Core settings of Amazon import new shipment status are deleted, please upgrade '
                    'Amazon module to back this settings.'))
            name = 'FBA-' + seller.name + ' : Amazon new inbound shipment check status'
            cron_values.update({'name': name})
            new_cron = auto_inbound_cron.copy(default=cron_values)
            model_name = 'ir_cron_auto_check_status_new_inbound_shipment_%d' % (seller.id)
            self.create_amazon_fba_scheduler(new_cron.id, model_name)
        return True
